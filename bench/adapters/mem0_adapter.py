...
class Mem0Adapter(MemoryAdapter):
    ...
    def __init__(self, llm_provider: str = "ollama", llm_model: str = _DEFAULT_LLM,
                 embed_model: str = _DEFAULT_EMB, ollama_url: str = _OLLAMA, **_):
        # Per-WORKER mem0 home, so concurrent workers don't collide on the shared
        # ~/.mem0 migrations-qdrant lock (this is what blocked workers>1). mem0
        # reads MEM0_DIR at import time, and this runs in __init__ before the
        # first `from mem0 import Memory` in reset() -> each worker process gets
        # its own .mem0_w<pid>. Enables real parallelism.
        os.environ["MEM0_DIR"] = os.path.join(os.path.expanduser("~"), f".mem0_w{os.getpid()}")
        # llm_provider: "ollama" (local qwen, handicapped) or "openai" (gpt-4o-mini,
        # mem0's blessed config = mem0-at-best). Embedder stays local nomic either
        # way (free + keeps the embedding comparison fair). "openai" reads the key
        # from the OPENAI_API_KEY env var -- never passed through code/args.
        self.llm_provider = llm_provider
        self.llm_model = "gpt-4o-mini" if (llm_provider == "openai" and llm_model == _DEFAULT_LLM) else llm_model
    ...
    def _config(self) -> dict:
        ...
        if self.llm_provider == "openai":
            llm_cfg = {"provider": "openai", "config": {
                "model": self.llm_model, "temperature": 0}}  # key via OPENAI_API_KEY env
        else:
            llm_cfg = {"provider": "ollama", "config": {
        ...
        return {
            ...
            "vector_store": {"provider": "qdrant", "config": {
                ...
                "path": os.path.join(self._item_dir, "qdrant"), "on_disk": True}},
        }
    ...
    def reset(self, item_dir: str) -> None:
        ...
        self.llm_tokens = 0
        # token-wrap only the Ollama client (chat is a callable method there).
        # OpenAI's client.chat is a namespace (chat.completions.create) -- wrapping
        # it breaks every call ('function' has no attribute 'completions'). Token
        # cost for mem0 is already captured by the local perf run.
        if self.llm_provider != "openai":
            self._wrap_tokens()

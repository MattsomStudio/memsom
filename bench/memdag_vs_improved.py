"""memdag_vs_improved — does the improved retrieval beat memdag's ACTUAL shipped
retrieval (BM25 + nomic-dense equal-RRF) by >= 0.1 hit@1, held-out?

This is the faithful product comparison. memdag_retrieve.retrieve() fuses BM25
(K1=1.2,B=0.75) with Ollama dense via _rrf_fuse (equal RRF). We replicate that
EXACTLY here (importing memdag's own tokenize), call it `memdag_current`, and pit
...
BENCH = r"C:\Users\you\memdag\bench"
...
def build_bm25(corpus_text):
    """In-memory BM25 over the corpus, faithful to memdag_retrieve.bm25."""
...
def main():
    ...
    for it in items:
        ...
        # memdag_current = RRF(bm25, nomic-dense) — the shipped fusion
        mcur = rrf_term(s_bm) + rrf_term(s_nom)
        memdag_rank = gold_rank(np.argsort(-mcur).tolist(), gold)
        cand = set()
        for s in sigs:
        ...
        if not (gold & set(cand)):
            cand_miss += 1
        Q.append({"gold": gold, "X": feats(sigs, cand), "cand": cand, "memdag_rank": memdag_rank})
    if q_new:
        pickle.dump(qcache, open(qfile, "wb"))
    print(f"[mvi] candidate gold miss = {cand_miss}/{len(Q)}", file=sys.stderr, flush=True)

    memdag_h1 = sum(1 for q in Q if q["memdag_rank"] == 1) / len(Q)
    ...
    for f in range(args.folds):
        ...
        for i in te:
            q = Q[i]
            h1m += 1.0 if q["memdag_rank"] == 1 else 0.0
        ...
        print(f"  fold {f}: n={n} memdag={h1m/n:.4f} learned={h1l/n:.4f} Δ={(h1l-h1m)/n:+.4f}", file=sys.stderr, flush=True)

    g = np.array(fold_gain)
    print("\n" + "=" * 66)
    print("  memdag SHIPPED (BM25+nomic RRF)  vs  LEARNED RERANKER  — hit@1")
    print("=" * 66)
    print(f"  memdag_current hit@1 (full)   = {memdag_h1:.4f}")
    ...
    if args.out:
        Path(args.out).write_text(json.dumps({
            "memdag_current_h1": memdag_h1, "learned_cv_h1": float(np.mean(fold_learn)),

...
md = MemdagAdapter(r"C:\Users\you\memdag")
import memdag, memdag_retrieve   # after adapter put repo on sys.path
...
conn=memdag.get_connection(); t=time.time(); rows=memdag_retrieve.retrieve(conn,e["question"],k=200); print(f"retrieve top200: {time.time()-t:.1f}s ({len(rows)} rows)"); conn.close()

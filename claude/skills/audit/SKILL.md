---
name: audit
description: Deterministic structural integrity audit of the Claude memory store (the flat per-fact files that feed memsom and the generated MEMORY.md index). Trigger on /audit, "audit my memory", "check memory integrity", "is my memory store healthy", "any broken links or orphans in memory". Catches orphan files, dead MEMORY.md links, broken frontmatter, bad types, broken wikilinks, and MEMORY.md budget breaches. No LLM, read-only.
---
...
- **orphan-file** and **dead-index-link** are the actionable ones. Because MEMORY.md
  is **generated** by `memsom bridge-render`, the fix is never to hand-edit the
  index — for an orphan, re-run `memsom bridge-render` (it re-renders from the
...
- `memsom bridge-render` — regenerates MEMORY.md from the store (clears orphan /

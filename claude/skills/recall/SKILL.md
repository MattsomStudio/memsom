---
name: recall
description: Search the memsom memory store by meaning, not just by what's loaded in MEMORY.md. Trigger when the user asks things like "what did we figure out about X", "recall that conversation about Y", "search my memory for Z", "what do I have on W", "remember when we worked on…", or any request to find prior content that isn't in the always-loaded MEMORY.md index. Runs memsom's hybrid (BM25 + local vector) retrieval over the whole store and returns ranked hits with their provenance.
---

# recall — semantic search over the memsom store
...
- To pull the full provenance of a promising hit: `memsom explain <id>`.
- To trace a hit back to its root sources: `memsom blame <id>`.

## What's actually searchable

`retrieve` searches the **memsom store** — one corpus, everything that's been
ingested:

- facts captured via `/saveall`,
- past chat history, if ingested: `memsom ingest-chats`,
- notes / docs / a folder of markdown, if ingested: `memsom ingest-dir <dir>`.
...
a large ingest, `memsom reindex` rebuilds the BM25 postings.
...
- `memsom explain` / `memsom blame` — provenance of a specific hit.

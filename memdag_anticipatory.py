"""memdag_anticipatory — anticipatory coprocess (Phase 2): retrieval-backed
...
spine's pool filters (memdag_cli._build_pool / memdag_retrieve.retrieve).
...
memdag.derive_node (label = min(parents)) with honest provenance edges, and
...
def migrate(conn):
    ...
    memdag_schema.ensure_table(conn, _PREFETCH_SQL)
...
def untainted_sources(conn, clearance="topsecret"):
    ...
    coprocess may compose from.  Same shape as memdag.live_sources:
...
def _tf_map(text):
    ...
    m = {}
    for t in memdag_retrieve.tokenize(text):
        m[t] = m.get(t, 0) + 1
    return m
...
def _vector_sims(text, candidates):
    ...
    (same pattern as memdag_retrieve.vector_search)."""
    if not candidates:
        return {}
    try:
        qv = memdag_retrieve._call_ollama_embed(text)
    except Exception:
        return {}
    out = {}
    for nid, content in candidates:
        try:
            cv = memdag_retrieve._call_ollama_embed(content)
        except Exception:
            continue
        out[nid] = memdag_retrieve._cosine(qv, cv)
    return out
...
def surprise_gated_write(conn, question, threshold=0.35, sources=None,
    ...
    else derive a new one (label = min(parents) via memdag.derive_node —
...
def prefetch(conn, k=3, threshold=0.35, clearance="topsecret", pool_k=8):
    ...
    real hybrid retrieval (memdag_retrieve.retrieve — untainted by
    ...
    for query, _count, _ts in rows:
        pool = memdag_retrieve.retrieve(conn, query, k=pool_k,
                                        clearance=clearance)
        if not pool:

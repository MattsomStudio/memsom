...
importer: it reads every memory/*.md and mirrors each into ONE memdag node.
...
and the _cmd_* wrapper do I/O.  Frozen core (memdag.py) is untouched — this is a
...
def import_literals(conn, memory_dir, *, dry_run: bool = True) -> dict:
    """Mirror the file-less MEMORY.md index lines into memdag as endorsed nodes.
    ...
    def _do():
        for sref, (section, payload) in desired.items():
            ...
            nid = memdag.insert_node(conn, content, "endorsed", source_ref=sref)
            conn.execute("UPDATE nodes SET content_hash = ? WHERE id = ?",
                         (memdag_ingest._content_hash(content), nid))
            ...
            if sref not in desired:
                ...
                if not dry_run:
                    conn.execute(
                        ...
                        "WHERE id = ?",
                        (memdag.now_iso(), "literal removed from index", nid))
...
def import_memory_dir(conn, memory_dir, *, dry_run: bool = True) -> dict:
    """Import every memory/*.md (excluding MEMORY.md) into memdag.
    ...
    def _do():
        for path in files:
            ...
            new_hash = memdag_ingest._content_hash(stamped)
            ...
            if existing:
                conn.execute(
                    "UPDATE nodes SET tombstoned = 1, tombstoned_at = ?, revoke_reason = ? "
                    "WHERE id = ?",
                    (memdag.now_iso(), "superseded by bridge reimport", existing[0]),
                )
                stats["tombstoned"] += 1
                stats["updated"] += 1
            else:
            ...
            nid = memdag.insert_node(conn, stamped, channel, source_ref=f"memory:{stem}")
            conn.execute(
...
def register(sub) -> None:
    ...
    p = sub.add_parser("bridge-import",
                       help="import flat-file memories into memdag (Phase 1)")
...
def main(argv=None) -> None:
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    ap = argparse.ArgumentParser(prog="memdag_bridge_import", description=__doc__)

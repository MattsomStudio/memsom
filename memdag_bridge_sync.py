"""memdag_bridge_sync — Mac<->PC memory sync over federation changesets (Phase 4).
...
the other machine's changeset into its own LOCAL DB.  memdag_federation already
...
import memdag
import memdag_federation as fed
...
def register(sub) -> None:
    ...
    pe.add_argument("dir", help="output dir (e.g. the synced claude-sync/memdag/)")
...
def main(argv=None) -> None:
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    ap = argparse.ArgumentParser(prog="memdag_bridge_sync", description=__doc__)

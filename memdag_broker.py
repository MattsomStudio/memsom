"""memdag_broker — Gate #3: a provenance-gating MCP broker (the action gate).

memdag's third gate.  The write gate (consolidation/quarantine) governs what an
...
min() (memdag_session).  A consequential tool (policy required > external) is
then DENIED — the call never reaches the upstream server.

Run as the `memdag-broker` console entry point (separate process from
`memdag-mcp`, so the frozen MCP server + its tests are untouched).
...
def load_config(path=None) -> dict:
    ...
    if not p.exists():
        raise FileNotFoundError(f"broker config not found: {p} (run `memdag broker-init`)")
    try:
...
def migrate(conn):
    ...
    memdag_schema.ensure_table(conn, _PIN_SQL)
...
def decide_and_forward(conn, cfg, policy, sid, fq, arguments, caller) -> dict:
    ...
    """
    required = memdag_policy.required_floor(policy, fq)
    floor = memdag_session.current_floor(conn, sid)
    verdict = memdag_capgate.check_capability(conn, sid, floor, fq, required)

    if verdict["decision"] == "deny":
        return _text_result(
            f"memdag-broker DENIED {fq}: session @{verdict['floor_name']} "
...
def aggregated_tools(conn, upstreams: dict) -> list:
    """Return the namespaced union of memdag.* tools and every upstream.* tool,
    minus any rug-pulled (schema-changed) upstream tool."""
    import memdag_mcp  # lazy — avoids a cli<->mcp<->broker import cycle
    out = []
    for t in memdag_mcp.TOOLS:
        nt = dict(t)
        nt["name"] = f"memdag.{t['name']}"
        out.append(nt)
...
def _handle(conn, cfg, policy, upstreams, sid, msg):
    ...
    if method == "tools/call":
        ...
        # memdag's own tools: dispatch in-process, ungated (read/store only).
        if fq.startswith("memdag."):
            inner = memdag_mcp.handle({
                "jsonrpc": "2.0", "id": msg_id, "method": "tools/call",
                "params": {"name": fq[len("memdag."):], "arguments": arguments}})
            return inner
        ...
        if tool in blocked:
            return {"jsonrpc": "2.0", "id": msg_id,
                    "result": _text_result(
                        f"memdag-broker REFUSED {fq}: tool schema changed since "
        ...
        try:
            ...
            print(f"[memdag-broker] error forwarding {fq}: {exc}", file=sys.stderr)
            return {"jsonrpc": "2.0", "id": msg_id,
...
# ---------------------------------------------------------------------------
# broker-init / policy-check CLI (mounted into the main `memdag` CLI)
# ---------------------------------------------------------------------------
...
def cmd_policy_check(args):
    ...
    policy = memdag_policy.load_policy(pol_path)
    req = memdag_policy.required_floor(policy, args.tool)
    conseq = memdag_policy.is_consequential(policy, args.tool)
    taint = memdag_policy.taints(policy, args.tool)
    req_name = memdag.NAME.get(req, "DENY")
    print(f"tool={args.tool}  required={req_name}  consequential={'yes' if conseq else 'no'}"
          + (f"  taints={memdag.NAME[taint]}" if taint is not None else ""))
...
def _selfcheck() -> int:
    ...
    try:
        policy = memdag_policy._normalize({
            "default": "deny",
            "rules": [
        ...
        sid = memdag_session.begin_session(conn, "user")
        ...
        if memdag_session.current_floor(conn, sid) != memdag.RANK["external"]:
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    ap = argparse.ArgumentParser(prog="memdag-broker",
    ...
    policy = memdag_policy.load_policy(cfg["policy"])
    conn = memdag.get_connection()
    upstreams = {}
    try:
        for name, spec in cfg["upstreams"].items():
            upstreams[name] = Upstream(name, spec).start()
        sid = memdag_session.begin_session(conn, cfg["session_start"])
        print(f"[memdag-broker] session {sid[:8]} @ {cfg['session_start']}; "

"""memdag_capgate — capability gate: the Gate #3 decision + audit (action gate).

The runtime sibling of memdag_gate.check_action (memdag_gate.py:98).  Where
check_action compares a NODE's integrity label against a required floor,
check_capability compares the SESSION's integrity floor (memdag_session) against
a TOOL's required floor (memdag_policy).  Same RANK vocabulary, same
...
def check_capability(conn, session_id, session_floor, tool, required) -> dict:
    ...
    *required*), log the decision, and return it.
    ...
    if session_floor >= required:
        decision = "allow"
        reason = f"session floor {memdag.NAME[session_floor]} >= required {req_name}"
    else:
        decision = "deny"
        reason = f"session floor {memdag.NAME[session_floor]} < required {req_name}"
    ...
    with conn:
        conn.execute(
            ...
            " VALUES (?,?,?,?,?,?,?)",
            (session_id, tool, req_name, session_floor, decision, reason, memdag.now_iso()),
        )

    return {
        "decision": decision,
        "floor": session_floor,
        "floor_name": memdag.NAME[session_floor],
        "required": required,
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_capgate")

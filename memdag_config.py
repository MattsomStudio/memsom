#!/usr/bin/env python3
"""memdag_config — safely wire `memdag-mcp` into MCP client config files.
...
def wire_json(path, abs_exe, db_path, print_only=False):
    ...
    snippet = json.dumps({"mcpServers": {"memdag": entry}}, indent=2)
    ...
    if not path.exists():
        ...
        path.write_text(json.dumps({"mcpServers": {"memdag": entry}}, indent=2) + "\n",
                        encoding="utf-8")
    ...
    if servers.get("memdag") == entry:
        return {"action": "unchanged", "path": str(path)}

    _backup(path)
    servers["memdag"] = entry  # merge: only our key changes, others untouched
...
def wire_toml(path, abs_exe, db_path, print_only=False):
    ...
    existing = parsed.get("mcp_servers", {}).get("memdag")
    if existing == want:
        return {"action": "unchanged", "path": str(path)}
...
def _default_exe():
    return shutil.which("memdag-mcp") or "memdag-mcp"


def register(subparsers):
    p = subparsers.add_parser("wire-config",
                              help="merge memdag into an MCP client's config (backup + idempotent)")
    p.add_argument("--client", choices=["claude-code", "claude-desktop", "codex", "all"],
                   default="all")
    p.add_argument("--exe", default=None, help="absolute path to memdag-mcp (default: resolve on PATH)")
    p.add_argument("--db", default=None, help="MEMDAG_DB path to wire (default: memdag's default)")
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(prog="memdag-wire-config")

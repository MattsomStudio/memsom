"""memdag_corroborate — Corroboration v1: registered-roots-only, capped at agent-derived(1), fail-closed.
...
def migrate(conn):
  ...
  added_at TEXT NOT NULL
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS claims (
  ...
  UNIQUE(subject, predicate, value)
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS claim_assertions (
  ...
  PRIMARY KEY (claim_id, node_id)
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS corroborations (
  claim_id INTEGER NOT NULL REFERENCES claims(id),
  node_id INTEGER NOT NULL REFERENCES nodes(id),
...
def corroborate(conn, claim_id, k=2):
    ...
    with conn:
        ...
        lift = memdag.insert_node(conn, content, "agent-derived",
                                  label=memdag.RANK["agent-derived"])
        assert lift is not None
        # Verify the cap is holding (belt-and-braces; catches refactoring accidents)
        assert memdag.RANK["agent-derived"] == 1, "cap constant drift — fix before proceeding"
        ...
        conn.execute(
            ...
            " VALUES (?,?,?,?,?)",
            (claim_id, lift, k, n_roots, memdag.now_iso())
        )
...
def cmd_assert_claim(args):
    ...
    try:
        ...
        if args.auto:
            ...
            if node is None:
                print(f"[memdag] unknown node id: {node_id}", file=sys.stderr)
                sys.exit(1)
            triple = extract_claim(node["content"])
            if triple is None:
                print(
                    f"[memdag] no structured claim found in node [{node_id}]"
            ...
            if not (args.subject and args.predicate and args.value):
                print(
                    "[memdag] explicit triple requires --subject, --predicate, and --value",
                    file=sys.stderr,
                )
                sys.exit(2)
...
def cmd_corroborate(args):
    ...
    try:
        ...
        if lift is not None:
            node = memdag.get_node(conn, lift)
...
def main(argv=None):
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_corroborate")

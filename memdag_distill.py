"""memdag_distill — provenance-filtered training-set export and distillation planner.
...
def _instruction_from_content(content):
    ...
    If content starts with 'Q: ' (the memdag.compose format), strip that prefix
...
def distill_plan(model=None, out_dir=None):
    ...
    out_dir : str or None  — destination directory; defaults to the memdag package dir.
    ...
    if out_dir is not None:
        ...
        out_dir = Path(memdag.__file__).parent
    ...
    ps1_lines = [
        "# distill.ps1 — memdag distillation runner stub",
        "# Generated by memdag_distill.distill_plan()",
        ...
        "python memdag_cli.py export-training training.jsonl --min-integrity agent-derived",
        "",
        ...
        "# model.save_pretrained('memdag-distilled-lora')",
        "# '@",
        "",
        ...
        "# ollama create memdag-distilled -f Modelfile",
        "",
    ...
    plan_text = "\n".join([
        ...
        f"",
        f"boundary note:    memdag guarantees WHAT goes into the weights — it exports only",
        ...
        f"                  is the one manual GPU step that memdag deliberately does not automate.",
        f"",
        f"next steps:",
        f"  1. Review {dataset_path} (export-training writes it)",
        f"  2. Run step 2 (GPU fine-tune) manually — see distill.ps1",
        f"  3. Uncomment step 3 in distill.ps1 and run `ollama create memdag-distilled -f Modelfile`",
    ])
    return plan_text
...
def register(subparsers):
    ...
    p_plan.add_argument("--out-dir", default=None, dest="out_dir",
                        help="directory for config + runner (default: memdag package dir)")
...
def main(argv=None):
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_distill")

#!/usr/bin/env python3
"""memdag_doctor — one command that dumps everything needed for a bug report.
...
Python version+path, memdag version, DB location + node count, and an Ollama
...
def _version():
    try:
        ...
        return im.version("memdag")
    except Exception:
        return "unknown (not installed as a package)"


def _node_count():
    # Read-only + busy_timeout: memdag runs in rollback-journal (not WAL) mode, so
    # a concurrent client writer could otherwise raise 'database is locked'.
    db = Path(memdag.db_path())
    if not db.exists():
        return None
    try:
...
def _selfcheck():
    try:
        r = subprocess.run([sys.executable, memdag_mcp.__file__, "--selfcheck"],
...
def gather():
    return {
        "memdag_version": _version(),
        "os": platform.platform(),
        "arch": platform.machine(),
        "python_version": platform.python_version(),
        "python_executable": sys.executable,
        "db_path": str(memdag.db_path()),
...
def _format(report):
    ...
    lines = [
        "memdag doctor",
        "=============",
        f"memdag version : {report['memdag_version']}",
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(prog="memdag-doctor")

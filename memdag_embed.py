"""memdag_embed — opt-in embedding-backend dispatch + BGE-M3 triple fusion.

memdag's retrieval is BM25 (stdlib) plus an OPTIONAL dense vector layer. This
...
imported at module top beyond stdlib + memdag_schema, so importing memdag_embed
...
memdag_schema.taint_filter_clauses (the pool gate). Sparse + ColBERT signals
...
the re-ranker. See memdag_retrieve.retrieve / colbert_rerank for the proof.
...
DEFAULT_COLBERT_CANDIDATES = 100   # ColBERT re-rank window (see memdag_retrieve)
...
def active_model_name() -> str:
    ...
    import memdag_retrieve  # lazy: avoid import cycle at module load
    return memdag_retrieve._embed_model()
...
def store_bge(conn: sqlite3.Connection, nid: int, enc: dict) -> None:
    ...
    """
    migrate(conn)
    import memdag_retrieve  # lazy: reuse the float32 dense packer (cycle-safe)

    dense = enc["dense"]
    dense_blob = memdag_retrieve._vec_to_blob(dense)
    import json

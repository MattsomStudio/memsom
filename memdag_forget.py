...
(test_memdag_forget.TestParity), which feeds the same inputs to both and asserts
...
Frozen core (memdag.py) is untouched; all new columns are additive + nullable.
...
# test_memdag_forget.TestParity (golden parity against the original).     #
# ====================================================================== #
...
def read_usage_events(usage_dir, after_ts=None):
    ...
    parameter (memdag is shippable — no hard-coded ~/.claude path).  Returns {}
...
# ====================================================================== #
# memdag storage adapter — state in node columns, inventory from nodes    #
# ====================================================================== #
...
def migrate(conn) -> None:
    ...
    for col, decl in _COLS:
        memdag_schema.add_column(conn, "nodes", col, decl)
    conn.execute(
...
def build_inventory(conn):
    ...
    memdag every cold node was demoted BY this layer — there is no hand-curated

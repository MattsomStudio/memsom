"""memdag_gate — action-time integrity floor enforcement (Move 4).
...
`check`, but that name is already registered by memdag_heal.register()
(memdag_heal.py line 317). Using it would cause argparse to raise
...
def migrate(conn):
    ...
    memdag_schema.ensure_table(conn, _GATE_LOG_SQL)
...
def _parse_required(value) -> int:
    ...
    lower = s.lower()
    if lower in memdag.RANK:
        return memdag.RANK[lower]
...
def check_action(conn, node_id, required_floor) -> dict:
    ...
    if floor >= req:
        ...
        culprit = None
        reason = (
            f"floor {memdag.NAME[floor]} >= required {memdag.NAME[req]}"
        )
    else:
        decision = "deny"
        # Walk leaf ancestors via blame (handles missing redacted/status columns)
        entries = memdag_blame.blame(conn, node_id)
    ...
    with conn:
        conn.execute(
            ...
            " VALUES (?,?,?,?,?,?)",
            (node_id, memdag.NAME[req], floor, decision, culprit, memdag.now_iso()),
        )
    ...
    return {
        "decision": decision,
        "floor": floor,
        "floor_name": memdag.NAME[floor],
        "required": req,
        "required_name": memdag.NAME[req],
        "culprit": culprit,
        "reason": reason,
    }
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_gate")

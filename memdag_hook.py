"""memdag_hook — Gate #3 native-tool arm via Claude Code hooks.

The broker (memdag_broker) guards MCP doors; this guards the doors Anthropic
...
memdag via memdag_session.ensure_session), so taint follows the live
conversation.  The shared brain is memdag_session + memdag_policy +
memdag_capgate — identical to the broker, different hands.

The hook is wired in settings.json (run `memdag hook-print-config`):
  "PostToolUse": [{"matcher": "WebFetch|WebSearch",
                   "hooks": [{"type": "command", "command": "memdag hook-post"}]}],
  "PreToolUse":  [{"matcher": "Bash|Edit|Write|MultiEdit",
                   "hooks": [{"type": "command", "command": "memdag hook-pre"}]}]
...
whenever memdag can read the session floor; only true internal faults fail open.
...
def load_hook_policy():
    ...
    if path is not None:
        try:
            return memdag_policy.load_policy(path)
        except Exception as exc:  # noqa: BLE001 — fall back, never disable the gate
            print(f"[memdag-hook] policy {path} unusable, using built-in default: {exc}",
                  file=sys.stderr)
    return memdag_policy._normalize(DEFAULT_HOOK_POLICY)
...
def decide_pre(conn, policy, session_id, tool) -> dict:
    ...
    memdag_session.ensure_session(conn, session_id, "user")
    floor = memdag_session.current_floor(conn, session_id)
    required = memdag_policy.required_floor(policy, tool)
    return memdag_capgate.check_capability(conn, session_id, floor, tool, required)
...
def apply_post(conn, policy, session_id, tool):
    ...
    taint = memdag_policy.taints(policy, tool)
    if taint is None:
        return None
    memdag_session.ensure_session(conn, session_id, "user")
    return memdag_session.lower_floor(conn, session_id, taint, tool, reason=f"hook:{tool}")
...
def pre_output(verdict, tool):
    ...
    return {"hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            f"memdag Gate #3: this session was tainted to {verdict['floor_name']} "
...
def cmd_hook_pre(args):
    ...
    try:
        ...
        print(f"[memdag-hook] pre error (failing open): {exc}", file=sys.stderr)
        return
...
def cmd_hook_post(args):
    ...
    try:
        ...
        print(f"[memdag-hook] post error: {exc}", file=sys.stderr)
        return
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_hook")

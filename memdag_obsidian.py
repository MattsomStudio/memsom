"""memdag_obsidian — optional Obsidian-vault integration for the derivation DAG.

An Obsidian vault is already a graph: every ``[[wikilink]]`` is an edge.  This
module maps that graph onto memdag's edge layer instead of slurping notes as
...
SECURITY — channel is stamped by PROVENANCE, never by "it is a file in the vault".
    The write-back path is a self-inflicted laundering vector: memdag writes an
    ...
    a note may carry `memdag-channel:` in frontmatter, but it can only ever
    *lower* integrity, never raise it:  effective = min(default, declared).
    So a memdag-authored note (stamped agent-derived) re-enters as agent-derived,
    ...
    memdag_ingest.ingest_text, so F-13 (channel ceiling) and F-14 (channel->label
    lock) still hold.
...
export_note(conn, vault, *, node_id=None, query=None, folder="memdag",
            title=None, clearance="topsecret") -> Path
    Write an answer back to the vault as a memdag-stamped note (refuses to
    overwrite a note memdag did not author).
...
CHANNEL_KEY = "memdag-channel"
AUTHORED_KEY = "memdag-authored"
SOURCES_KEY = "memdag-source-nodes"
...
def migrate(conn) -> None:
    ...
    memdag_schema.add_column(conn, "nodes", "obsidian_path", "TEXT")
    memdag_schema.add_column(conn, "nodes", "obsidian_mtime", "TEXT")
    conn.execute(
...
def parse_frontmatter(text: str):
    ...
    Only a tiny, predictable subset — enough for tags/aliases/memdag-* keys.
    """
...
def sync_vault(conn, vault, default_channel="user", prune=True, log=None):
    ...
    for rel, ap, mtime_ns, size in files:
        ...
        for nid in _live_nodes_for_path(conn, rel):
            memdag.revoke_cascade(conn, nid, f"obsidian: note changed ({rel})")

        channel = effective_channel(default_channel, fm.get(CHANNEL_KEY))
        ids = memdag_ingest.ingest_text(conn, body, channel, source_ref=rel)
        if ids:
            qmarks = ",".join("?" * len(ids))
            with conn:
                conn.execute(
    ...
    for rel, targets in note_links.items():
        ...
        for tgt in targets:
            ...
            if dst is None or dst == src:
                continue
            try:
                memdag_relate.relate(conn, src, dst, kind=LINK_KIND)
                summary["edges"] += 1
            except ValueError:
    ...
    if prune:
        for rel in _all_known_paths(conn):
            ...
            if (vault / rel).exists():
                continue
            for nid in _live_nodes_for_path(conn, rel):
                memdag.revoke_cascade(conn, nid, f"obsidian: note deleted ({rel})")
            summary["deleted"] += 1
...
# ---------------------------------------------------------------------------
# Write-back: export an answer to the vault as a memdag-stamped note
# ---------------------------------------------------------------------------
...
def _compose_answer(conn, query: str, clearance: str):
    ...
    ids are returned (so memdag-source-nodes and the backlinks are populated).
    """
    import memdag_cli  # lazy: memdag_cli imports this module at its top level
    pool = memdag_cli._build_pool(conn, clearance)
    if not pool:
        return "", []
    text, used = memdag.compose(query, pool)
    if not text:
        return "", []
...
def _note_is_memdag_authored(path: Path) -> bool:
    try:
    ...
    # Require BOTH the authored flag AND the source-nodes key memdag always
    # writes, so a hand-written note that merely happens to carry
    # `memdag-authored: true` is not silently overwritable.
    return val in ("true", "yes", "1") and SOURCES_KEY in fm


def export_note(conn, vault, *, node_id=None, query=None, folder="memdag",
                title=None, clearance="topsecret"):
    """Write an answer back into the vault as an agent-derived, memdag-stamped note.

    Exactly one of node_id / query must be given.  Returns the written Path.
    Refuses to write outside the vault or to overwrite a note memdag did not
    author.  The `memdag-channel: agent-derived` stamp is what lets the reader
    ...
    if out_path.exists() and not _note_is_memdag_authored(out_path):
        raise ValueError(
            f"refusing to overwrite a note memdag did not author: {out_path}"
        )
...
def _cmd_watch(args) -> None:
    vault = _default_vault(args.vault)
    watch_vault(
        memdag.get_connection, vault, args.channel,
...
def register(subparsers) -> None:
    ...
    p_sync.add_argument("--channel", default="user", choices=list(memdag.RANK.keys()),
    ...
    p_exp = subparsers.add_parser(
        "obsidian-export", help="write an answer back to the vault as a memdag note"
    )
    ...
    p_exp.add_argument("--folder", default="memdag", help="subfolder within the vault")
    ...
    p_watch.add_argument("vault", nargs="?", default=None,
                         help=f"vault path (default: ${VAULT_ENV})")
    p_watch.add_argument("--channel", default="user", choices=list(memdag.RANK.keys()))
...
def main(argv=None) -> None:
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag-obsidian")

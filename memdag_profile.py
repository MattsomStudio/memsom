"""memdag_profile — leaf-origin PROFILE for the derivation DAG.
...
gates — and it is enforced elsewhere (memdag_gate), never here.
...
# memdag.CASCADE_CTE — terminates on cycles, visits diamonds once).
# ---------------------------------------------------------------------------
...
def profile(conn, nid):
    ...
    if node["channel"] != "agent-derived":
        ...
        has_red = memdag_schema.column_exists(conn, "nodes", "redacted")
...
def cmd_profile(args):
    ...
    try:
        ...
        for label in sorted(p["hist"], reverse=True):
            print(f"  {memdag.NAME[label]:<13} {p['hist'][label]}")
        if p["external_leaf_ids"]:
...
def main(argv=None):
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_profile")

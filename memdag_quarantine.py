"""memdag_quarantine — consolidation gates + quarantine.
...
def migrate(conn: sqlite3.Connection) -> None:
    ...
    memdag_schema.add_column(conn, "nodes", "status",
...
def live_source_ids(conn: sqlite3.Connection) -> list:
    ...
    memdag.live_sources is the source of truth for which rows are sources;
...
def live_unquarantined_sources(conn: sqlite3.Connection) -> list:
    """Same row shape as memdag.live_sources (id, content, channel, label, source_ref),
...
def _cmd_quarantine(args) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"[memdag-quarantine] {exc}", file=sys.stderr)
            sys.exit(1)
        if flipped:
    ...
    try:
        ...
        if not rows:
            ...
            for r in rows:
                print(
                    f"[{r['id']}] {r['channel']:<13}"
                    f" integrity={memdag_schema.NAME[r['label']]:<13}"
...
def main(argv=None) -> None:
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag-quarantine")

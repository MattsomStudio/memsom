"""memdag_redact — shape-preserving REDACTION (Guarantee #6 second mode).
...
def redact_node(conn, nid, reason, cascade=False):
    ...
    if cascade:
        targets_raw = memdag.cascade_set(conn, nid)
...
def live_unredacted_sources(conn):
    """Live, non-redacted sources — same row shape as memdag.live_sources.
...
def describe(conn, nid):
    """Render a node like memdag.fmt_node, replacing the snippet with a redaction
    ...
    # Build a node dict compatible with memdag.fmt_node
    node = {
    ...
    lines = memdag.fmt_node(node)

    if redacted:
        local = memdag.local_date(redacted_at)
...
def cmd_redact(args):
    ...
    try:
        migrate(conn)
        node = memdag.get_node(conn, args.id)
        if node is None:
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag-redact")

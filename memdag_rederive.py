...
def migrate(conn):
    ...
    """
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS derivation_recipe (
...
def _replay(recipe, live_parents):
    ...
    if engine == "compose":
        ...
        text, _used = memdag.compose(question, sources)
        return text
    if engine == "extractive":
        import memdag_compact  # lazy: compact imports this module at top
        rows = sorted(((p[0], p[1]) for p in live_parents), key=lambda r: r[0])  # id asc
        return memdag_compact.extractive_summary(rows, kw.get("k", _EXTRACTIVE_DEFAULT_K))
    return None
...
def regenerate(conn, node_id):
    ...
    new_id, _label = memdag.derive_node(conn, new_content, live_ids)  # commits its own txn
...
def erase(conn, node_id, reason):
    ...
    """
    import memdag_redact  # lazy: avoids any import-order coupling
    migrate(conn)                         # derivation_recipe must exist for the chain walk
    erased = set()
    for target in _version_chain(conn, node_id):
        erased.update(memdag_redact.redact_node(conn, target, reason, cascade=True))

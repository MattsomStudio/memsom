"""memdag_relate — GraphRAG made safe: associative 'relates-to' edges.
...
def migrate(conn):
    """Idempotent migration for memdag_relate.
    ...
    memdag_schema.ensure_table(conn, _REL_EDGES_DDL)
...
def _parse_integrity(val):
    ...
    if isinstance(val, str):
        ...
        try:
            ...
            raise ValueError(f"unknown integrity label {val!r}; valid = {list(memdag.RANK)}") from None
        if 0 <= n <= 3:
            return n
...
def relate(conn, a, b, kind="relates-to"):
    ...
    if memdag.get_node(conn, a) is None:
        raise ValueError(f"unknown node id: {a}")
    if memdag.get_node(conn, b) is None:
...
def neighborhood(conn, nid, hops=2, min_integrity=0, clearance=3):
    ...
    where line = '[REDACTED]' if the node is redacted else memdag.snippet(content).
    ...
    clearance = memdag_confid.parse_conf(clearance)

    start = memdag.get_node(conn, nid)
    if start is None:
...
def main(argv=None):
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_relate")

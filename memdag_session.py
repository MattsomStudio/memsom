"""memdag_session — per-session integrity taint floor (Gate #3 substrate).

A "session" is one run of the memdag broker (memdag_broker.py): the process
...
(memdag.py:33  endorsed3 > user2 > agent-derived1 > external0).  It starts HIGH
(default `user`) and only ever DROPS — the runtime dual of derive_node's
`min(parent labels)` (memdag.py:122).  When untrusted content flows through the
...
def _parse_floor(value) -> int:
    ...
    otherwise.  Mirrors memdag_gate._parse_required but kept local so this
    ...
    if s.isdigit():
        ...
        if 0 <= n <= 3:
            return n
        raise ValueError(f"bad floor: {value!r}")
    if s.lower() in memdag.RANK:
        return memdag.RANK[s.lower()]
    raise ValueError(f"bad floor: {value!r}")
...
def begin_session(conn, start_floor=None) -> str:
    ...
    *start_floor* defaults to RANK['user'] (2): a session begins with
    ...
    """
    floor = _parse_floor(start_floor) if start_floor is not None else memdag.RANK["user"]
    sid = uuid.uuid4().hex
    now = memdag.now_iso()
    migrate(conn)
    with conn:
        ...
        if not rows:
            ...
            for r in rows:
                print(
                    ...
                    f"  {memdag.NAME[r['from_floor']]} -> {memdag.NAME[r['to_floor']]}"
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_session")

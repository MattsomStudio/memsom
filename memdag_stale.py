"""memdag_stale — STALENESS CASCADE (verifiable correctness-over-time).

The fourth flag on a node, orthogonal to the three security flags:

  - tombstoned = revoked (liveness dies)            [memdag.py]
  - redacted   = payload destroyed                  [memdag_redact.py]
  - quarantined/archived = consolidation states     [memdag_quarantine/compact]
...
memdag_schema.taint_filter_clauses (the always-on dead filter); it lives here as
...
Mechanism (reuses the revocation-cascade engine, aimed at a new trigger):
  ...
  2. mark_stale_cascade marks the old version + every transitive descendant stale
     (the memdag.revoke_cascade twin: same CASCADE_CTE walk, sets stale=1 not
  ...
  4. freshen(node) — the ONLY thing that rewrites provenance — rewires the stale
     node's edge old->fresh source, then regenerates (memdag_rederive.regenerate),
...
def migrate(conn):
    ...
    memdag_schema.add_column(conn, "nodes", "stale",        "INTEGER NOT NULL DEFAULT 0")
    memdag_schema.add_column(conn, "nodes", "stale_at",     "TEXT")
    memdag_schema.add_column(conn, "nodes", "stale_reason", "TEXT")
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS source_supersedes (
  ...
  CREATE INDEX IF NOT EXISTS idx_source_supersedes_ref ON source_supersedes(source_ref);""")
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS stale_log (
...
# ---------------------------------------------------------------------------
# Cascade engine (the memdag.revoke_cascade twin: marks stale, never kills)
# ---------------------------------------------------------------------------

def mark_stale_cascade(conn, seed, reason):
    """Mark *seed* and every transitive descendant STALE (not tombstoned).

    Structurally identical to memdag.revoke_cascade: one atomic UPDATE over the
...
def stale_set(conn, seed):
    ...
    migrate(conn)
    return conn.execute(
        memdag.CASCADE_CTE + " SELECT n.id, n.channel, COALESCE(n.stale, 0) FROM nodes n"
...
def superseding_version(conn, old_id):
    ...
    if not memdag_schema.column_exists(conn, "nodes", "stale"):
...
def stale_exclude_clauses(conn):
    ...
    deliberately SEPARATE from memdag_schema.taint_filter_clauses: staleness is
    ...
    """
    if memdag_schema.column_exists(conn, "nodes", "stale"):
        return (["stale = 0"], [])
    return ([], [])
...
def stale_annotations(conn, node_ids):
    ...
    """
    if not node_ids or not memdag_schema.column_exists(conn, "nodes", "stale"):
        return {}
...
def _is_live_source(conn, nid):
    ...
    for col in ("redacted", "archived"):
        if memdag_schema.column_exists(conn, "nodes", col):
...
def freshen_preview(conn, node_id):
    ...
    """
    migrate(conn)
    if memdag.get_node(conn, node_id) is None:
        raise ValueError(f"unknown node id: {node_id}")
...
def freshen(conn, node_id):
    ...
    audit row per rewire); (2) memdag_rederive.regenerate(node) replays the recipe
    ...
    new_id = memdag_rederive.regenerate(conn, node_id)
    if new_id is None:
...
def cmd_stale_cascade(args):
    ...
    try:
        migrate(conn)
        if memdag.get_node(conn, args.id) is None:
            print(f"[memdag-stale] no node [{args.id}]", file=sys.stderr)
            sys.exit(1)
...
def cmd_stale_status(args):
    ...
    try:
        ...
        print(f"[memdag-stale] {exc}", file=sys.stderr)
        sys.exit(1)
    finally:
        conn.close()
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag-stale")

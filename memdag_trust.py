"""memdag_trust — integrity lattice (meet/join) + audited manual elevation.

The ONLY legal way a node's label rises is through elevate().  Every elevation
is recorded in the `elevations` audit table so that memdag_recompute treats the
...
Invariants upheld here:
  ...
  - After elevation, memdag_recompute.recompute_all() re-floors all descendants
...
def migrate(conn):
    ...
    memdag_schema.ensure_table(conn, """CREATE TABLE IF NOT EXISTS elevations (
...
def elevate(conn, nid, to_label, reason, by, force=False):
    ...
    changed = memdag_recompute.recompute_all(conn)
...
def cmd_elevate(args):
    ...
    try:
        ...
        try:
            ...
            sys.exit(1)
        from_name = memdag.NAME[result["from"]]
        to_name = memdag.NAME[result["to"]]
        ...
        for nid, old, new in result["changed_descendants"]:
            print(f"  re-floored [{nid}] {memdag.NAME[old]} -> {memdag.NAME[new]}")
    finally:
        conn.close()
        ...
        if not rows:
            ...
            for r in rows:
                print(f"[{r['id']}] node={r['node']}  "
                      f"{memdag.NAME[r['from_label']]} -> {memdag.NAME[r['to_label']]}  "
...
def main(argv=None):
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memdag_trust")

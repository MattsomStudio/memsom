"""memdag_verify_stale — verification-age staleness for bridge memory notes.

A NEW TRIGGER feeding the memdag_stale engine (which owns mark/clear/supersede).
Where memdag_stale's cascade fires on "the source CONTENT changed" (content_hash
...
Design:
  ...
  - REASON-NAMESPACE OWNERSHIP (the critical safety boundary): this pass only ever
    ...
    supersession-cascade flag from memdag_stale is left untouched.
...
def recompute_verify_stale(conn, *, now=None, threshold_days=None, dry_run=False):
    ...
    """
    memdag_stale.migrate(conn)                    # defensive (also done in bi.migrate)
    now = now or _now()
...
def main(argv=None) -> None:
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    ap = argparse.ArgumentParser(prog="memdag_verify_stale", description=__doc__)

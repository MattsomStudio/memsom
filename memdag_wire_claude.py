#!/usr/bin/env python3
"""memdag_wire_claude — install the Claude Code memory loop for a tester.

Sibling of memdag_config (the MCP wiring); same safety contract: BACK UP first,
MERGE never overwrite, IDEMPOTENT, malformed config -> refuse + print. Three pieces:
  ...
  2. STOP HOOK— merge a Stop hook that runs `<memdag> bridge-render` into
                ~/.claude/settings.json (regenerates MEMORY.md from the store on
                session end). Deduped on re-run; everything else preserved.
  3. CLAUDE.md— seed/refresh the memdag-managed memory block (via memdag_claude).
...
def default_skills_src():
    # Works when running from the repo (python memdag_cli.py wire-claude); bootstrap
    # passes an explicit --skills-src for the installed case.
    return Path(__file__).resolve().parent / "claude" / "skills"


def _default_exe():
    return shutil.which("memdag") or "memdag"
...
def gate_event_groups(abs_exe):
    """The opt-in Gate #3 taint hooks (mirrors memdag_hook._CONFIG_SNIPPET)."""
    return {
...
# ---------------------------------------------------------------------------
# settings.json IO (mirrors memdag_config.wire_json contract)
# ---------------------------------------------------------------------------
...
def wire_claude(*, home=None, abs_exe=None, skills_src=None, with_gate=False,
    ...
    # memdag_claude resolve its default ($CLAUDE_MD_PATH or ~/.claude/CLAUDE.md).
    import memdag_claude
    claude_path = (Path(home) / ".claude" / "CLAUDE.md") if home else None
    if print_only:
        out["claude_md"] = {"action": "print", "snippet": memdag_claude.render_block()}
    else:
        try:
            out["claude_md"] = memdag_claude.sync(path=claude_path)
...
def register(subparsers):
    ...
    p.add_argument("--exe", default=None,
                   help="absolute path to the memdag executable (default: resolve on PATH)")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(prog="memdag-wire-claude")

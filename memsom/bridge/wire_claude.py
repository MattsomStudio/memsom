...
def memory_dir_candidates(home=None):
    ...
    override = memsom_tuning.resolve("bridge.memory_dir") if home is None else ""
    if override:
        p = Path(override)
...
def scaffold_memory(home=None, *, print_only=False):
    ...
    # FAILOPEN: allowed, a scaffold failure is reported back in the result dict, never raised.
...
def wire_claude(*, home=None, abs_exe=None, skills_src=None, with_gate=False,
    ...
    if print_only:
        ...
        # FAILOPEN: allowed, a CLAUDE.md sync failure is reported back, never raised.
        except Exception as exc:

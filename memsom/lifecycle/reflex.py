"""memsom_reflex — reflex/schema-shaped training export (Phase 3 PoC).
...
CONSOLIDATED memory is eligible.  Eligibility reuses memsom_distill's
...
archived by memsom_compact).  Nothing external / quarantined / redacted /
...
# IS the experiment.
REFLEX_SYSTEM = "You are memsom-reflex, Matt's memory-grounded lab assistant."
...
def consolidated_ids(conn):
    ...
    Signature of memsom_compact.compact(): the minted node's immediate parents
...
def eligible_consolidated(conn, min_integrity=1):
    ...
    Mirrors memsom_distill.export_training's belt-and-braces selection
...
def format_context(items):
    ...
    for it in items:
        blocks.append(
            f"[mem:{it['id']}|agent-derived|{memsom.NAME[it['label']]}]"
            f"\n{it['content']}"
        )
    return "\n\n".join(blocks)
...
def build_house_answer(nid, content, label, n_parents):
    ...
    return (
        f"Verdict: {verdict}\n"
        f"Evidence:\n{evidence}\n"
        f"Integrity: {memsom.NAME[label]} (floor of {n_parents} episodes)\n"
        f"Next move: treat as {memsom.NAME[label]} - run `memsom explain {nid}` "
...
def main(argv=None):
    ...
    for s in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    p = argparse.ArgumentParser(prog="memsom_reflex")

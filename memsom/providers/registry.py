...
from memsom.providers.kokoro import KokoroAdapter
...
# kinds that manage a local serving process → need the ProcessManager. Parakeet
# is handed the procman for parity with the serving adapters (it runs in-process
# so never actually spawns one). Kokoro is a CPU ONNX model with no server, so it
# does not need the procman at all.
_PROC_KINDS = {"llamacpp", "vllm", "parakeet"}
...
def _construct(kind: str, spec: dict, procman):
    ...
    if kind == "kokoro":
        return KokoroAdapter(spec, procman=procman)
    return None

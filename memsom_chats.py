#!/usr/bin/env python3
"""memsom_chats — opt-in ingestion of a user's OWN chat history.
    ...
    for path in files:
        ...
        for rec in records:
            ...
            if not dry_run:
                ...
                memsom_ingest.ingest_text(conn, rec["text"], channel,
...
def register(subparsers):
    p = subparsers.add_parser(
        "ingest-chats",
        help="opt-in: seed memsom from your OWN local chat history (channel=user)")
...
def cmd_ingest_chats(args):
    if args.file and args.client == "all":
        print("[memsom] --file requires an explicit --client (claude-code or codex)",
              file=sys.stderr)
        sys.exit(1)
    ...
    if total == 0:
        print("[memsom] no local transcripts found for "
    ...
    if not args.yes and not args.dry_run:
        ...
        resp = input("Ingest these into memsom (channel=user)? [y/N] ").strip().lower()
        if resp not in ("y", "yes"):
            print("[memsom] aborted; nothing ingested.", file=sys.stderr)
            sys.exit(0)

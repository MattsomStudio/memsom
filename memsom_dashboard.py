#!/usr/bin/env python3
"""
memsom_dashboard.py — visual telemetry dashboard for the Claude memory store.

Reads the LIVE forgetting state from memsom's forget_* columns (~/.memdag/
...
from memsom_bridge_import import default_memory_dir, split_frontmatter, fm_top_level
...
def _memsom_db():
    # Live forgetting state = memsom's forget_* columns. Resolved at call time
...
def load_weights() -> list[dict]:
    """Live forgetting telemetry from memsom's forget_* columns (the bridge's
...
def render(telemetry: dict, out: Path):
    ...
    # <script> block (json.dumps does not escape slashes). memsom ingests untrusted

"""memsom_recompute — multi-hop integrity recompute (the cut 'min beyond one hop').
...
def _elevated_ids(conn):
    ...
    Detects the table WITHOUT importing memsom_trust (no circular import).
...
def effective_labels(conn):
    ...
    memsom_heal's integrity-mismatch check flags exactly the same rows.
    """
    elevated = _elevated_ids(conn)
    memo = {}
    rows = conn.execute(
        ...
        if has_all:
            ...
            if not changes:
                ...
                for nid, old, new in changes:
                    print(f"[{nid}] AGENT-DERIVED  {memsom.NAME[old]} -> {memsom.NAME[new]}")
        else:
            nid = args.id
            try:
                ...
                sys.exit(1)
            if old == new:
                print(f"[{nid}] unchanged ({memsom.NAME[old]})")
            else:
                print(f"[{nid}] AGENT-DERIVED  {memsom.NAME[old]} -> {memsom.NAME[new]}")
    finally:
        conn.close()

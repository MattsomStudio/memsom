"""memsom_schema — shared idempotent migration helpers.

Used by every memsom_* feature module.  NO CLI, no register(), no DB access at
...
def _step_status_check(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        # memsom (ids are never reused), so the rebuilt table must continue the
...
def taint_filter_clauses(conn: sqlite3.Connection, clearance=None,
    ...
    Semantics (mirrors the spine's pool filters; security load-bearing):
      ...
      - status != 'quarantined' when the column exists, unless the caller
                                ...
                                (only memsom_retrieve's exclude_quarantined=False
    ...
    DELIBERATELY EXCLUDED: `stale` (memsom_stale). Staleness is NOT a security
    ...
    in memsom_stale.stale_exclude_clauses and is appended only on the fresh-only path.
    """

...
class TestWireToml(unittest.TestCase):
    ...
    def test_appends_block_when_absent(self):
        ...
        res = memsom_config.wire_toml(self.path, EXE, DB)
        self.assertEqual(res["action"], "merged")
        p = self._parsed()
        self.assertEqual(p["mcp_servers"]["memsom"]["command"], EXE)
        # env lives in the SUB-TABLE, not an env_vars array
        self.assertEqual(p["mcp_servers"]["memsom"]["env"]["MEMDAG_DB"], DB)

    def test_preserves_existing_toml(self):
        self.path.write_text('model = "gpt-5"\n\n[mcp_servers.other]\ncommand = "x"\n',
                             encoding="utf-8")
        memsom_config.wire_toml(self.path, EXE, DB)
        p = self._parsed()
        self.assertEqual(p["model"], "gpt-5")
        self.assertIn("other", p["mcp_servers"])
        self.assertIn("memsom", p["mcp_servers"])

    def test_idempotent_toml(self):
        self.path.write_text('model = "gpt-5"\n', encoding="utf-8")
        memsom_config.wire_toml(self.path, EXE, DB)
        first = self.path.read_bytes()
        res2 = memsom_config.wire_toml(self.path, EXE, DB)
    ...
    def test_db_path_with_special_chars(self):
        ...
        cases = [
            (r"C:\Users\foo\.memdag\venv\Scripts\memsom-mcp.exe",
             r"C:\Users\foo\.memdag\memdag.db"),
            ("/Users/foo/Library/Application Support/x/bin/memsom-mcp",
             "/Users/foo/.memdag/memdag.db"),
        ]
        for exe, db in cases:
            with self.subTest(exe=exe):
                p = Path(self.tmp.name) / f"cfg_{abs(hash(exe))}.toml"
                res = memsom_config.wire_toml(p, exe, db)
                self.assertEqual(res["action"], "created")
                parsed = tomllib.loads(p.read_text(encoding="utf-8"))
                self.assertEqual(parsed["mcp_servers"]["memsom"]["command"], exe)
                self.assertEqual(parsed["mcp_servers"]["memsom"]["env"]["MEMDAG_DB"], db)


class TestWireTomlInlineTable(unittest.TestCase):
    """CONFIG-MERGE-INLINE-1: appending a [mcp_servers.memsom] header to a file

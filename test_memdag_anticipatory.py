...
Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memdag_anticipatory.py \
    -t <repo> -v
"""
...
class Phase2Base(Base):
    ...
    def derive_answer(self, question, sources=None):
        ...
        nid, created, _ = memdag_anticipatory.surprise_gated_write(
            self.conn, question, sources=sources)
        self.assertTrue(created, "helper expects a fresh mint")
        return nid, memdag.get_node(self.conn, nid)["content"]
...
class TestSemanticSurprise(Phase2Base):
    ...
    def test_disjoint_topic_scores_high(self):
        ...
        score = memdag_anticipatory.surprise(self.conn, probe)
    ...
    def test_rank_similar_orders_identical_first(self):
        ...
        n1, t1 = self.derive_answer(
            ...
            sources=[(s1, SRC_NEBULA, "endorsed", memdag.RANK["endorsed"], None)])
        n2, _ = self.derive_answer(
            "How do I grow tomatoes in my garden?",
            sources=[(s2, SRC_GARDENING, "user", memdag.RANK["user"], None)])
        ranked = memdag_anticipatory.rank_similar(self.conn, t1)
...
class TestSemanticDedupBeyondExactHash(Phase2Base):
    ...
    def test_reworded_question_cites_existing(self):
        ...
        n1, created1, _ = memdag_anticipatory.surprise_gated_write(self.conn, q1)
        self.assertTrue(created1)
        t1 = memdag.get_node(self.conn, n1)["content"]

        n2, created2, score2 = memdag_anticipatory.surprise_gated_write(self.conn, q2)
        t2_probe, _ = memdag.compose(
            q2, memdag_anticipatory.untainted_sources(self.conn))
...
class TestPrefetchCacheServeWarm(Phase2Base):
    ...
    def _warm_one(self):
        ...
        return memdag_anticipatory.prefetch(self.conn, k=1)
    ...
    def test_prefetch_populates_cache_and_serves_warm(self):
        ...
        warm = memdag_anticipatory.serve_warm(self.conn, self.Q)
        ...
        warm2 = memdag_anticipatory.serve_warm(self.conn, self.Q)
        self.assertEqual(warm2["hits"], 2)

    def test_unknown_query_is_a_miss(self):
        self._warm_one()
        self.assertIsNone(
            memdag_anticipatory.serve_warm(self.conn, "never asked this"))

    def test_prefetch_is_idempotent_one_cache_row(self):
        self._warm_one()
        results2 = memdag_anticipatory.prefetch(self.conn, k=1)
    ...
    def test_revoked_answer_is_never_served_warm(self):
        results = self._warm_one()
        nid = results[0][1]
        memdag.revoke_cascade(self.conn, nid, "answer recalled")
        self.assertIsNone(memdag_anticipatory.serve_warm(self.conn, self.Q),
...
class TestNovelRecombination(Phase2Base):
    ...
    def test_unseen_set_is_novel_then_seen(self):
        novel, prior = memdag_anticipatory.novel_recombination(
            self.conn, [self.a, self.b])
        self.assertTrue(novel)
        self.assertEqual(prior, [])

        d1, _ = memdag.derive_node(self.conn, "combined claim", [self.a, self.b])
        novel, prior = memdag_anticipatory.novel_recombination(
            self.conn, [self.a, self.b])
    ...
    def test_exact_set_semantics_subset_superset_novel(self):
        memdag.derive_node(self.conn, "combined claim", [self.a, self.b])
        self.assertTrue(memdag_anticipatory.novel_recombination(
    ...
    def test_exclude_node_and_duplicate_ids(self):
        d1, _ = memdag.derive_node(self.conn, "combined claim", [self.a, self.b])
        novel, _ = memdag_anticipatory.novel_recombination(
...
class TestStatusSnapshot(Phase2Base):
    def test_status_counts_and_validity(self):
        self.add(SRC_NEBULA, "endorsed")
        q = "How should I configure the Nebula mesh lighthouse?"
        memdag_anticipatory.observe(self.conn, q)
        memdag_anticipatory.observe(self.conn, q)
        memdag_anticipatory.observe(self.conn, "one-off question")
        results = memdag_anticipatory.prefetch(self.conn, k=1)
        nid = results[0][1]

        s = memdag_anticipatory.status(self.conn)
        ...
        memdag.revoke_cascade(self.conn, nid, "recalled")
        s2 = memdag_anticipatory.status(self.conn)
...
class TestCoprocessNeverTouchesTainted(Phase2Base):
    ...
    def test_tainted_sources_never_in_pool(self):
        ...
        memdag.revoke_cascade(self.conn, s_tomb, "gone")
        memdag_quarantine.quarantine_node(self.conn, s_quar, "poisoned")
        memdag_redact.redact_node(self.conn, s_red, "secret")
        with self.conn:
            self.conn.execute(
                "UPDATE nodes SET archived=1 WHERE id=?", (s_arch,))
        memdag_confid.classify(self.conn, s_secret, "secret")

        ids = {r[0] for r in memdag_anticipatory.untainted_sources(
        ...
        ids_top = {r[0] for r in memdag_anticipatory.untainted_sources(self.conn)}
    ...
    def test_poisoned_derived_never_in_surprise_corpus_or_cited(self):
        ...
        taints = [
            lambda nid: memdag_quarantine.quarantine_node(self.conn, nid, "poison"),
            lambda nid: memdag_redact.redact_node(self.conn, nid, "poison"),
            lambda nid: memdag.revoke_cascade(self.conn, nid, "poison"),
            lambda nid: self.conn.execute(
                "UPDATE nodes SET archived=1 WHERE id=?", (nid,)),
        ]
        for taint in taints:
            nid, created, _ = memdag_anticipatory.surprise_gated_write(self.conn, q)
            self.assertTrue(
                created,
                "a poisoned prior answer must never be CITED — gate must mint")
            text = memdag.get_node(self.conn, nid)["content"]
            # Sanity: while live, the answer IS the corpus match
            self.assertLess(memdag_anticipatory.surprise(self.conn, text), 0.05)
            with self.conn:
                taint(nid)
            # Poisoned: gone from the corpus entirely
            corpus_ids = {i for i, _ in
                          memdag_anticipatory.untainted_derived(self.conn)}
            self.assertNotIn(nid, corpus_ids)
            self.assertEqual(
                memdag_anticipatory.rank_similar(self.conn, text), [],
                "a poisoned node must not appear in any surprise comparison")
            self.assertAlmostEqual(
                memdag_anticipatory.surprise(self.conn, text), 1.0)

    def test_above_clearance_answer_never_cited_below_clearance(self):
        self.add(SRC_NEBULA, "endorsed")
        q = "How should I configure the Nebula mesh lighthouse?"
        nid, _, _ = memdag_anticipatory.surprise_gated_write(self.conn, q)
        memdag_confid.classify(self.conn, nid, "secret")
        corpus = {i for i, _ in memdag_anticipatory.untainted_derived(
            self.conn, clearance="internal")}
        self.assertNotIn(nid, corpus,
                         "a secret answer must be invisible below clearance")
        n2, created2, _ = memdag_anticipatory.surprise_gated_write(
    ...
    def test_external_tainted_derivation_never_cited_or_served(self):
        ...
        n1, created1, _ = memdag_anticipatory.surprise_gated_write(self.conn, q)
        self.assertTrue(created1)
        self.assertEqual(memdag.get_node(self.conn, n1)["label"], 0,
        ...
        self.assertEqual(memdag_anticipatory.untainted_derived(self.conn), [])
        n2, created2, _ = memdag_anticipatory.surprise_gated_write(self.conn, q)
        ...
        with self.conn:
            self.conn.execute(
                ...
                " VALUES (?,?,?)", (q, n1, memdag.now_iso()))
        self.assertIsNone(memdag_anticipatory.serve_warm(self.conn, q),
    ...
    def test_prefetch_never_composes_from_poisoned_source(self):
        ...
        memdag_quarantine.quarantine_node(self.conn, s_poison, "poisoned advice")
        ...
        content = memdag.get_node(self.conn, nid)["content"]
    ...
    def test_recombination_never_lists_tainted_prior(self):
        ...
        d1, _ = memdag.derive_node(self.conn, "combined claim", [a, b])
        memdag_quarantine.quarantine_node(self.conn, d1, "poisoned inference")
        novel, prior = memdag_anticipatory.novel_recombination(self.conn, [a, b])

"""Tests for memdag_audit — structural integrity audit of the flat memory store.

Run:  python -m unittest discover -s . -p test_memdag_audit.py
"""
...
class TestDBUnavailableDegrades(Base):
    def test_orphan_downgrades_to_warn_without_store(self):
        ...
        orig = memdag.get_connection
        memdag.get_connection = lambda *a, **k: (_ for _ in ()).throw(RuntimeError("no db"))
        try:
            findings, _f, (_t, db_ok) = audit.run_audit(self.mem)
        finally:
            memdag.get_connection = orig

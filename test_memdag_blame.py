#!/usr/bin/env python3
"""Tests for memdag_blame — derivation-DAG blame ("git blame") module.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_blame.py \
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        memdag_blame.migrate(self.conn)  # ensure redacted + status columns
...
class TestBlameListsExactRoots(Base):
    ...
    def test_blame_lists_exact_roots(self):
        ...
        result = memdag_blame.blame(self.conn, d)
...
class TestMultihopReachesRoots(Base):
    ...
    def test_multihop_reaches_roots(self):
        ...
        result = memdag_blame.blame(self.conn, d3)
...
class TestRedactedAncestorShownWithState(Base):
    ...
    def test_redacted_ancestor_shown_with_state(self):
        ...
        # Redact the root
        memdag_redact.redact_node(self.conn, e, reason="test redaction")
        ...
        for line in memdag_blame.format_blame(self.conn, d):
...
class TestBlameOfSourceIsItself(Base):
    ...
    def test_blame_of_source_is_itself(self):
        ...
        result = memdag_blame.blame(self.conn, e)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["id"], e)
        self.assertEqual(result[0]["channel"], "endorsed")
...
class TestUnknownIdRaises(Base):
    ...
    def test_unknown_id_raises(self):
        with self.assertRaises(ValueError):
            memdag_blame.blame(self.conn, 9999)

...
Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memdag_broker_poc.py -t <repo> -v
"""
...
class TestBrokerPoC(unittest.TestCase):
    ...
    def test_poisoned_fetch_blocks_subsequent_send(self):
        ...
        self.assertEqual(memdag_session.current_floor(self.conn, self.sid), memdag.RANK["user"])
        ...
        self.assertFalse(r["result"]["isError"])
        self.assertEqual(memdag_session.current_floor(self.conn, self.sid), memdag.RANK["external"])
        ev = memdag_session.session_log(self.conn, self.sid)
        ...
        self.assertEqual(len(memdag_capgate.recent_capability_log(self.conn)), 3)

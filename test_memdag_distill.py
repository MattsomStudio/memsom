#!/usr/bin/env python3
"""Tests for memdag_distill — provenance-filtered training-set export.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_distill.py \
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        memdag_distill.migrate(self.conn)
...
class TestExternalDerivedExcluded(Base):
    ...
    def test_external_derived_excluded(self):
        ...
        d_ext, _ = memdag.derive_node(self.conn, "answer from external", [ext])

        # At min_integrity=1: excluded because label=0
        records = memdag_distill.export_training(self.conn, min_integrity=1)
    ...
    def _export_ids(self, min_integrity=1):
        records = memdag_distill.export_training(self.conn, min_integrity=min_integrity)
...
class TestRedactedExcluded(Base):
    ...
    def test_redacted_excluded(self):
        ...
        memdag_redact.redact_node(self.conn, d, reason="test redaction")
        ids = self._export_ids()
...
class TestQuarantinedExcluded(Base):
    ...
    def test_quarantined_excluded(self):
        ...
        d, _ = memdag.derive_node(self.conn, "answer derived from user source quarantine", [u])

        ids = self._export_ids()
        self.assertIn(d, ids, "live node must appear before quarantine")

        memdag_quarantine.quarantine_node(self.conn, d, reason="suspicious")
        ids = self._export_ids()
...
class TestMinIntegrityHonored(Base):
    ...
    def test_min_integrity_honored(self):
        ...
        d_user, _ = memdag.derive_node(self.conn, "answer from user source for integrity check", [u])
        self.assertEqual(memdag.get_node(self.conn, d_user)["label"], 2)
        ...
        d_endorsed, _ = memdag.derive_node(self.conn, "answer from endorsed source for integrity check", [e])
        self.assertEqual(memdag.get_node(self.conn, d_endorsed)["label"], 3)

        def ids_at(floor):
            records = memdag_distill.export_training(self.conn, min_integrity=floor)
            result = set()
            for r in records:
                row = self.conn.execute(
                    "SELECT id FROM nodes WHERE content=? AND channel='agent-derived'",
...
class TestValidJsonlAndInstructionRecovery(Base):
    ...
    def test_valid_jsonl_and_instruction_recovery(self):
        ...
        parent_ids = {r[0] for r in memdag.parents_of(self.conn, nid)}
        ...
        memdag_distill.write_jsonl(out_path, records)
...
class TestRevokedExcluded(Base):
    ...
    def test_revoked_excluded(self):
        ...
        d, _ = memdag.derive_node(self.conn, "answer to be revoked for distill test", [u])

        ids = self._export_ids()
        self.assertIn(d, ids, "live node must appear before revoke")

        memdag.revoke_cascade(self.conn, d, "bad answer")
        ids = self._export_ids()
...
class TestDistillPlanWritesStub(Base):
    ...
    def test_distill_plan_writes_stub(self):
        ...
        src = Path(memdag_distill.__file__).read_text(encoding="utf-8")
        self.assertNotIn("subprocess", src,
                         "memdag_distill must not import subprocess")
        ...
        with patch.object(shutil, "which", return_value="/usr/local/bin/ollama") as mock_which:
            plan = memdag_distill.distill_plan(model=test_model, out_dir=str(out_dir))
        ...
        with patch.object(shutil, "which", return_value=None):
            plan_no_ollama = memdag_distill.distill_plan(model=test_model, out_dir=str(out_dir))

#!/usr/bin/env python3
"""Tests for memdag_embed (BGE-M3 triple-fusion backend) and its wiring into
memdag_retrieve.
...
`memdag_embed.bge_available` -> True and `encode_doc`/`encode_query` -> canned
...
Run:
  python -W error::DeprecationWarning -m unittest test_memdag_embed -v
"""
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        memdag_embed.migrate(self.conn)
...
class TestEmbedDispatch(unittest.TestCase):
    ...
    def test_default_is_ollama(self):
        os.environ.pop("MEMDAG_EMBED_BACKEND", None)
        self.assertEqual(memdag_embed.backend(), "ollama")

    def test_honors_env(self):
        os.environ["MEMDAG_EMBED_BACKEND"] = "bge-m3"
        self.assertEqual(memdag_embed.backend(), "bge-m3")
        os.environ["MEMDAG_EMBED_BACKEND"] = "bm25"
        self.assertEqual(memdag_embed.backend(), "bm25")
    ...
    def test_active_model_name(self):
        os.environ["MEMDAG_EMBED_BACKEND"] = "bge-m3"
        self.assertEqual(memdag_embed.active_model_name(), "bge-m3")
        os.environ["MEMDAG_EMBED_BACKEND"] = "bm25"
        self.assertEqual(memdag_embed.active_model_name(), "")
        os.environ["MEMDAG_EMBED_BACKEND"] = "ollama"
        self.assertEqual(memdag_embed.active_model_name(),
                         memdag_retrieve._embed_model())

    def test_colbert_candidates_default_and_override(self):
        os.environ.pop("MEMDAG_COLBERT_CANDIDATES", None)
        self.assertEqual(memdag_embed.colbert_candidates(), 100)
        os.environ["MEMDAG_COLBERT_CANDIDATES"] = "7"
        try:
            self.assertEqual(memdag_embed.colbert_candidates(), 7)
        finally:
            os.environ.pop("MEMDAG_COLBERT_CANDIDATES", None)
...
class TestPrimitives(unittest.TestCase):
    def test_colbert_blob_roundtrip(self):
        ...
        blob = memdag_embed.colbert_to_blob(mat)
        # 3 tokens * 2 dim * 2 bytes (fp16) = 12 bytes
        self.assertEqual(len(blob), 12)
        back = memdag_embed.blob_to_colbert(blob, 3, 2)
        self.assertEqual(back, mat)  # all values exactly fp16-representable

    def test_colbert_blob_empty(self):
        self.assertEqual(memdag_embed.blob_to_colbert(b"", 0, 1024), [])

    def test_sparse_dot_shared_keys(self):
        self.assertEqual(memdag_embed.sparse_dot({"a": 2.0, "b": 3.0},
                                                 {"b": 4.0, "c": 5.0}), 12.0)

    def test_sparse_dot_disjoint_is_zero(self):
        self.assertEqual(memdag_embed.sparse_dot({"a": 1.0}, {"b": 1.0}), 0.0)

    def test_sparse_dot_empty(self):
        self.assertEqual(memdag_embed.sparse_dot({}, {"a": 1.0}), 0.0)

    def test_colbert_maxsim_known(self):
        # Each query token's best doc match is 1.0; summed over 2 tokens = 2.0.
        q = [[1.0, 0.0], [0.0, 1.0]]
        d = [[1.0, 0.0], [0.0, 1.0]]
        self.assertAlmostEqual(memdag_embed.colbert_maxsim(q, d), 2.0, places=5)

    def test_colbert_maxsim_empty(self):
        self.assertEqual(memdag_embed.colbert_maxsim([], [[1.0]]), 0.0)
        self.assertEqual(memdag_embed.colbert_maxsim([[1.0]], []), 0.0)
...
class TestBgeDegradation(Base):
    ...
    def test_unavailable_falls_back_to_ollama(self):
        ...
        with patch.object(memdag_embed, "bge_available", lambda: False), \
             patch.object(memdag_retrieve, "_call_ollama_embed",
        ...
        self.assertEqual(row[0], memdag_retrieve._embed_model())
    ...
    def test_encoder_failure_falls_back_to_ollama(self):
        ...
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_doc", lambda t: None), \
             patch.object(memdag_retrieve, "_call_ollama_embed",
    ...
    def test_bm25_backend_stores_no_vectors(self):
        ...
        with patch.object(memdag_retrieve, "_call_ollama_embed",
...
class TestModelCollisionFix(Base):
    ...
    def test_ollama_query_ignores_bge_rows(self):
        ...
        self._plant(nid_nomic, memdag_retrieve._embed_model(), [1.0, 0.0, 0.0, 0.0])
        self._plant(nid_bge, "bge-m3", [1.0, 0.0, 0.0, 0.0])
        os.environ["MEMDAG_EMBED_BACKEND"] = "ollama"
        with patch.object(memdag_retrieve, "_call_ollama_embed",
                          lambda t, timeout=10: [1.0, 0.0, 0.0, 0.0]):
            res = memdag_retrieve.vector_search(self.conn, "q", k=5)
    ...
    def test_bge_query_ignores_ollama_rows(self):
        nid_nomic = self.add("alpha", "user")
        nid_bge = self.add("beta", "user")
        self._plant(nid_nomic, memdag_retrieve._embed_model(), [1.0, 0.0])
        self._plant(nid_bge, "bge-m3", [1.0, 0.0])
        os.environ["MEMDAG_EMBED_BACKEND"] = "bge-m3"
        enc_q = lambda t: _enc([1.0, 0.0], {}, [[1.0, 0.0]])
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_query", enc_q):
            res = memdag_retrieve.vector_search(self.conn, "q", k=5)
        ids = [r[0] for r in res]
...
class TestTripleFusion(Base):
    def test_store_bge_writes_all_three_tables(self):
        ...
        enc = lambda t: _enc([1.0, 0.0, 0.0, 0.0], {"42": 0.7, "7": 0.3},
                             [[1.0, 0.0], [0.0, 1.0]])
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_doc", enc):
            memdag_retrieve.index_node(self.conn, nid)
    ...
    def test_colbert_rerank_reorders_within_candidates(self):
        ...
        os.environ["MEMDAG_EMBED_BACKEND"] = "bge-m3"
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_doc", enc_doc):
            for nid in ids:
                memdag_retrieve.index_node(self.conn, nid)
        ...
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_query", enc_q):
            res = memdag_retrieve.retrieve(self.conn, "shared term", k=2)
    ...
    def test_colbert_rerank_is_pool_only(self):
        ...
        with patch.object(memdag_embed, "bge_available", lambda: True), \
             patch.object(memdag_embed, "encode_query",
                          lambda t: _enc([1.0], {}, [[1.0]])):
            os.environ["MEMDAG_EMBED_BACKEND"] = "bge-m3"
            out = memdag_retrieve.colbert_rerank(self.conn, "q", cand)
...
class TestDeindexPurgesAllThree(Base):
    ...
    def test_deindex_node_purges_bge_tables(self):
        ...
        memdag_retrieve.deindex_node(self.conn, nid)
        self.assertEqual(self.count("sparse_vecs", nid), 0)
        self.assertEqual(self.count("colbert_vecs", nid), 0)
    ...
    def test_redact_cascade_purges_bge_tables(self):
        ...
        self._index_bge(nid)
        with self.conn:
            memdag_redact.redact_node(self.conn, nid, "test redaction", cascade=True)
        self.assertEqual(self.count("sparse_vecs", nid), 0)
        self.assertEqual(self.count("colbert_vecs", nid), 0)
    ...
    def test_deindex_bge_noop_without_tables(self):
        ...
        try:
            memdag_embed.deindex_bge(bare, 1)  # must not raise
        finally:
            bare.close()
...
class TestSecurityInvariant(Base):
    ...
    def test_above_clearance_node_excluded_despite_strong_colbert(self):
        ...
        with patch.object(memdag_embed, "bge_available", lambda: True), \
            ...
            res_low = memdag_retrieve.retrieve(self.conn, "shared", k=5, clearance="public")
            ...
            # not a bug).
            res_hi = memdag_retrieve.retrieve(self.conn, "shared", k=5, clearance="topsecret")
...
class TestRrfNary(unittest.TestCase):
    def test_two_arg_matches_manual(self):
        a = [(1, 5.0), (2, 4.0)]
        b = [(2, 0.9), (3, 0.8)]
        fused = dict(memdag_retrieve._rrf_fuse(a, b))
    ...
    def test_three_arg_fuses(self):
        ...
        b = [(2, 1.0)]
        c = [(1, 1.0), (2, 1.0)]
        fused = dict(memdag_retrieve._rrf_fuse(a, b, c))
        self.assertEqual(set(fused.keys()), {1, 2})

    def test_rrf_c_keyword(self):
        a = [(1, 1.0)]
        fused = dict(memdag_retrieve._rrf_fuse(a, rrf_c=10))

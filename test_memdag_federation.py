#!/usr/bin/env python3
"""Tests for memdag_federation — multi-machine sync with monotonic death/redact.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_federation.py \
...
class Base(unittest.TestCase):
    ...
    memdag.db_path() — but tests use the explicit connection objects directly.
    """
    ...
    def setUp(self):
        ...
        self.conn_a = memdag.get_connection(self.db_a)
        self.conn_b = memdag.get_connection(self.db_b)
    ...
    def add(self, conn, content, channel):
        with conn:
            return memdag.insert_node(conn, content, channel, memdag.RANK[channel])

    def export_file(self, conn, suffix="cs.jsonl", **kw):
        path = str(Path(self.tmp.name) / suffix)
        cs = memdag_federation.export_changeset(conn, **kw)
        memdag_federation.write_jsonl(path, cs)
        return path, cs

    def import_file(self, conn, path):
        cs = memdag_federation.read_jsonl(path)
        return memdag_federation.import_changeset(conn, cs)
...
class TestRoundTripPreservesGraph(Base):
    ...
    def test_round_trip_preserves_graph(self):
        ...
        srcs = memdag.live_sources(self.conn_a)
        text, used = memdag.compose(q, srcs)
        if text is None:
...
class TestTombstoneNoPropagation(Base):
    ...
    def test_tombstone_propagates_no_resurrection(self):
        ...
        b, _ = memdag.derive_node(self.conn_a, "derived answer from source.", [a])
        ...
        # Revoke node b on A
        memdag.revoke_cascade(self.conn_a, b, "bad answer")
        ...
        stale_cs = memdag_federation.read_jsonl(path_stale)

        stats_stale_b = memdag_federation.import_changeset(self.conn_b, stale_cs)
        ...
        stats_stale_a = memdag_federation.import_changeset(self.conn_a, stale_cs)
...
class TestRedactionPropagates(Base):
    ...
    def test_redaction_propagates(self):
        ...
        stale_cs = memdag_federation.read_jsonl(path_stale)
        stats_stale = memdag_federation.import_changeset(self.conn_b, stale_cs)
...
class TestReimportIdempotent(Base):
    ...
    def test_reimport_idempotent(self):
        ...
        srcs = memdag.live_sources(self.conn_a)
        text, used = memdag.compose("What is the system?", srcs)
        if text is None:
            text = "derived"
            used = [e, u]
        memdag.derive_node(self.conn_a, text, used)
...
class TestQuarantinePropagatesPromoteIsLocal(Base):
    ...
    def test_quarantine_propagates_but_promote_is_local(self):
        ...
        d, _ = memdag.derive_node(self.conn_a, "derived from user source.", [n])
        ...
        memdag_quarantine.quarantine_node(self.conn_a, d, "suspicious content found")
        ...
        for node in cs_b["nodes"]:
            if node.get("uuid") == uuid_d:
                node["status"] = "live"
        stats_live_over_quarantine = memdag_federation.import_changeset(self.conn_b, cs_b)
...
class TestBackfillIdempotent(Base):
    ...
    def test_backfill_idempotent_and_uuid_format(self):
        ...
        origin = "test-machine"
        count1 = memdag_federation.backfill_uuids(self.conn_a, origin)
        ...
        count2 = memdag_federation.backfill_uuids(self.conn_a, origin)
...
class TestSinceFilter(Base):
    ...
    def test_since_filter(self):
        ...
        d, _ = memdag.derive_node(self.conn_a, "derived for since filter.", [n])

        # Export with a far-future since -> 0 nodes
        future = "2099-01-01T00:00:00+00:00"
        cs_empty = memdag_federation.export_changeset(
        ...
        just_before = memdag.now_iso()

        # Revoke d -> sets tombstoned_at
        memdag.revoke_cascade(self.conn_a, d, "test revoke for since filter")

        # Export since=just_before -> should include at minimum d (tombstoned_at >= just_before)
        cs_since = memdag_federation.export_changeset(
...
class TestWriteReadJsonlRoundtrip(Base):
    ...
    def test_write_read_jsonl_roundtrip(self):
        ...
        d, _ = memdag.derive_node(self.conn_a, "derived from endorsed.", [n])

        path, cs_orig = self.export_file(self.conn_a, "jsonl_rt.jsonl", origin="machine-α")

        cs_loaded = memdag_federation.read_jsonl(path)
        ...
        with self.assertRaises(ValueError):
            memdag_federation.read_jsonl(bad_path)

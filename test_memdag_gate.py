#!/usr/bin/env python3
"""Tests for memdag_gate — action-time integrity floor enforcement.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_gate.py \
...
class TestAllowWhenFloorMeetsRequired(Base):
    def test_allow_when_floor_meets_required(self):
        ...
        d, _ = memdag.derive_node(self.conn, "derived from endorsed and user", [e, u])
        # floor = min(3, 2) = 2 = USER

        r = memdag_gate.check_action(self.conn, d, "user")
...
class TestDenyNamesWeakestLiveLeaf(Base):
    def test_deny_names_weakest_live_leaf(self):
        ...
        d, _ = memdag.derive_node(self.conn, "derived from endorsed and external", [e, x])
        # floor = min(3, 0) = 0
...
class TestCheckActionNeverMutatesLabels(Base):
    def test_check_action_never_mutates_labels(self):
        ...
        d_allow, _ = memdag.derive_node(self.conn, "allow derived node text", [e, u])
        d_deny, _ = memdag.derive_node(self.conn, "deny derived node text", [e, x])
        ...
        memdag_gate.check_action(self.conn, d_allow, "user")
        memdag_gate.check_action(self.conn, d_deny, "user")
...
class TestGateLogRowPerCall(Base):
    def test_gate_log_row_per_call(self):
        ...
        x = self.add("external source content here", "external")
        u = self.add("user source content here", "user")
        d, _ = memdag.derive_node(self.conn, "derived content text here", [e, x])

        memdag_gate.check_action(self.conn, d, "external")   # allow
        memdag_gate.check_action(self.conn, d, "user")       # deny
        memdag_gate.check_action(self.conn, u, "user")       # allow (source node)
...
class TestUnknownNodeRaisesAndNoLog(Base):
    def test_unknown_node_raises_and_no_log(self):
        ...
        with self.assertRaises(ValueError):
            memdag_gate.check_action(self.conn, 9999, "user")
...
class TestBadRequiredNameRaises(Base):
    def test_bad_required_name_raises(self):
        ...
        with self.assertRaises(ValueError):
            memdag_gate.check_action(self.conn, d, "banana")
...
class TestReadPathsAreFree(Base):
    def test_read_paths_are_free(self):
        ...
        memdag.compose("a question about external content", sources)
        memdag.get_node(self.conn, d)
        memdag.parents_of(self.conn, d)
...
class TestSourceNodeIsOwnCulprit(Base):
    def test_source_node_is_own_culprit(self):
        ...
        r = memdag_gate.check_action(self.conn, x, "endorsed")
...
class TestDenyAfterCulpritTombstonedFallsToNext(Base):
    def test_deny_after_culprit_tombstoned_falls_to_next(self):
        e = self.add("endorsed source content here", "endorsed")   # label 3
        x = self.add("external source content here", "external")   # label 0
        d, _ = memdag.derive_node(self.conn, "derived content text", [e, x])
        # floor stored = 0
        ...
        r = memdag_gate.check_action(self.conn, d, "endorsed")
    ...
    def test_cli_check_action_exit_codes(self):
        ...
        try:
            with contextlib.redirect_stdout(buf2):
                memdag_gate.main(["check-action", str(d_allow), "--require", "user"])
            exit_code = 0

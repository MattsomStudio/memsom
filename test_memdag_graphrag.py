...
Run:  python -m unittest discover -s . -p test_memdag_graphrag.py
"""
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        self._patcher = patch(
            "memdag_retrieve._call_ollama_embed", side_effect=Exception("offline"))
        self._patcher.start()
    ...
    def add(self, content, channel="user"):
        with self.conn:
            nid = memdag.insert_node(self.conn, content, channel, memdag.RANK[channel])
...
class TestPromotion(Base):
    def test_link_promotes_subcutoff_relevant_node(self):
        ...
        plain = self.ids(memdag_retrieve.retrieve(self.conn, "quux", k=2))
        ...
        memdag_relate.relate(self.conn, a, b, kind="wikilink")
        graph = self.ids(memdag_retrieve.retrieve_graph(self.conn, "quux", k=2))
    ...
    def test_promotion_survives_excluded_high_ranker(self):
        ...
        memdag_relate.relate(self.conn, a, b, kind="wikilink")
        graph = self.ids(memdag_retrieve.retrieve_graph(
...
class TestNoInjection(Base):
    def test_irrelevant_neighbor_never_injected(self):
        ...
        memdag_relate.relate(self.conn, a, z, kind="wikilink")
        graph = self.ids(memdag_retrieve.retrieve_graph(self.conn, "quux", k=8))
...
class TestTrustGate(Base):
    ...
    def _link_then_graph(self, a, b, clearance="topsecret", k=8):
        memdag_relate.relate(self.conn, a, b, kind="wikilink")
        return self.ids(
            memdag_retrieve.retrieve_graph(self.conn, "quux", k=k, clearance=clearance))
    ...
    def test_redacted_neighbor_not_leaked(self):
        a = self.add(_A)
        b = self.add(_B)
        memdag_redact.redact_node(self.conn, b, "test", cascade=False)
        graph = self._link_then_graph(a, b)
        self.assertNotIn(b, graph)
    ...
    def test_tombstoned_neighbor_not_leaked(self):
        a = self.add(_A)
        b = self.add(_B)
        memdag.revoke_cascade(self.conn, b, "test")
        graph = self._link_then_graph(a, b)
        self.assertNotIn(b, graph)
    ...
    def test_agent_derived_neighbor_not_leaked(self):
        ...
        a = self.add(_A)
        with self.conn:
            d = memdag.insert_node(self.conn, _B, "agent-derived", memdag.RANK["agent-derived"])
        memdag_retrieve.index_node(self.conn, d)  # no-op for agent-derived
...
class TestPoolSubsetInvariant(Base):
    ...
    (memdag_retrieve.py) — removing that gate makes these assertions fail. The
    ...
    def test_output_subset_of_pool_under_hostile_edges(self):
        ...
        with self.conn:
            ...
            d = memdag.insert_node(self.conn, _B, "agent-derived",
                                   memdag.RANK["agent-derived"])  # relevant but derived
        # Hostile edges from the clean seed to every excluded node.
        for bad in (above, quar, d):
            memdag_relate.relate(self.conn, a, bad, kind="wikilink")

        clr = memdag_confid.parse_conf("public")
        pool = memdag_retrieve._build_retrieve_pool(
            self.conn, clr, None, True, True)
        out = set(self.ids(
            memdag_retrieve.retrieve_graph(self.conn, "quux", k=8, clearance="public")))
...
class TestGraceful(Base):
    def test_empty_rel_edges_equals_retrieve(self):
        ...
        for k in (1, 2, 3):
            self.assertEqual(
                self.ids(memdag_retrieve.retrieve_graph(self.conn, "quux", k=k)),
                self.ids(memdag_retrieve.retrieve(self.conn, "quux", k=k)),
                f"graph diverged from retrieve with no edges (k={k})",
            )

    def test_zero_k_returns_empty(self):
        self.add(_A)
        self.assertEqual(memdag_retrieve.retrieve_graph(self.conn, "quux", k=0), [])

    def test_empty_pool_returns_empty(self):
        # No source nodes -> empty.
        self.assertEqual(memdag_retrieve.retrieve_graph(self.conn, "quux", k=8), [])

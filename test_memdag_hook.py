#!/usr/bin/env python3
"""Tests for memdag_hook — Gate #3 native-tool arm (Claude Code hooks).

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memdag_hook.py -t <repo> -v
"""
...
class TestEnsureSession(Base):
    def test_creates_at_user_then_idempotent(self):
        ...
        self.assertEqual(memdag_session.ensure_session(self.conn, sid), memdag.RANK["user"])
        # taint it, then ensure again must NOT reset it
        memdag_session.lower_floor(self.conn, sid, "external", "WebFetch", "t")
        self.assertEqual(memdag_session.ensure_session(self.conn, sid), memdag.RANK["external"])
...
class TestHookChain(Base):
    ...
    def test_webfetch_taints_then_bash_denied(self):
        ...
        self.assertEqual(new, memdag.RANK["external"])
...
class TestCli(Base):
    def _run(self, verb, payload):
        ...
        return subprocess.run(
            [sys.executable, "memdag_hook.py", verb],
    ...
    def test_malformed_stdin_fails_open(self):
        env = dict(os.environ)
        r = subprocess.run([sys.executable, "memdag_hook.py", "hook-pre"],

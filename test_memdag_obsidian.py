"""Tests for memdag_obsidian — Obsidian vault integration.

Run:  python -m unittest discover -s . -p test_memdag_obsidian.py
"""
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        memdag_obsidian.migrate(self.conn)
    ...
    def live_for(self, rel):
        return memdag_obsidian._live_nodes_for_path(self.conn, rel)

    def channel_of(self, nid):
        return memdag.get_node(self.conn, nid)["channel"]
...
class TestFrontmatter(Base):
    def test_no_frontmatter(self):
        fm, body = memdag_obsidian.parse_frontmatter("just text\nmore")
        self.assertEqual(fm, {})
    ...
    def test_scalars_and_lists(self):
        text = ("---\n"
                "title: My Note\n"
                "memdag-channel: agent-derived\n"
                "tags:\n  - alpha\n  - beta\n"
                "aliases: [AH, AltName]\n"
                "---\n"
                "body line one\n")
        fm, body = memdag_obsidian.parse_frontmatter(text)
        self.assertEqual(fm["title"], "My Note")
        self.assertEqual(fm["memdag-channel"], "agent-derived")
...
class TestChannelGuard(Base):
    def test_no_declaration_uses_default(self):
        self.assertEqual(memdag_obsidian.effective_channel("user", None), "user")

    def test_declared_can_lower(self):
        self.assertEqual(
            memdag_obsidian.effective_channel("user", "agent-derived"), "agent-derived")
        self.assertEqual(
            memdag_obsidian.effective_channel("user", "external"), "external")

    def test_declared_cannot_raise(self):
        # default=user(2); a note claiming endorsed(3) is clamped to user.
        self.assertEqual(
            memdag_obsidian.effective_channel("user", "endorsed"), "user")
        # default=external(0); claiming user is clamped to external.
        self.assertEqual(
            memdag_obsidian.effective_channel("external", "user"), "external")

    def test_garbage_declaration_ignored(self):
        self.assertEqual(
            memdag_obsidian.effective_channel("user", "totally-bogus"), "user")
...
class TestExtractLinks(Base):
    def test_plain_alias_heading_block(self):
        ...
        self.assertEqual(
            memdag_obsidian.extract_links(body), ["Alpha", "Beta", "Gamma", "Delta"])

    def test_embed_counts_as_link(self):
        self.assertEqual(memdag_obsidian.extract_links("![[Embedded]]"), ["Embedded"])

    def test_same_file_anchor_skipped(self):
        self.assertEqual(memdag_obsidian.extract_links("[[#Heading]] and [[#^blk]]"), [])

    def test_path_qualified(self):
        self.assertEqual(
            memdag_obsidian.extract_links("[[folder/Note]]"), ["folder/Note"])

    def test_links_in_code_fence_skipped(self):
        body = "real [[Real]]\n```\nfake [[NotReal]]\n```\nafter [[Also]]"
        self.assertEqual(memdag_obsidian.extract_links(body), ["Real", "Also"])

    def test_links_in_inline_code_skipped(self):
        body = "use `[[NotALink]]` but [[YesLink]] counts"
        self.assertEqual(memdag_obsidian.extract_links(body), ["YesLink"])

    def test_links_in_html_comment_skipped(self):
        body = "<!-- [[Hidden]] -->\nvisible [[Shown]]"
        self.assertEqual(memdag_obsidian.extract_links(body), ["Shown"])

    def test_escaped_brackets_skipped(self):
        body = r"escaped \[\[NotLink\]\] and [[Link]]"
        self.assertEqual(memdag_obsidian.extract_links(body), ["Link"])
    ...
    def test_markdown_link_local_and_external(self):
        ...
        self.assertEqual(memdag_obsidian.extract_links(body), ["My Note.md"])

    def test_frontmatter_quoted_wikilink(self):
        fm = {"related": '"[[FromFrontmatter]]"'}
        self.assertIn("FromFrontmatter", memdag_obsidian.extract_links("body", fm))

    def test_dedup_preserves_order(self):
        self.assertEqual(
            memdag_obsidian.extract_links("[[A]] [[B]] [[A]]"), ["A", "B"])
...
class TestResolver(Base):
    def test_unique_basename_resolves(self):
        by_name, by_rel = memdag_obsidian._build_resolver(["a/Note.md", "b/Other.md"])
        self.assertEqual(memdag_obsidian._resolve_target("Note", by_name, by_rel), "a/Note.md")

    def test_case_insensitive(self):
        by_name, by_rel = memdag_obsidian._build_resolver(["Note.md"])
        self.assertEqual(memdag_obsidian._resolve_target("note", by_name, by_rel), "Note.md")

    def test_ambiguous_basename_unresolved(self):
        by_name, by_rel = memdag_obsidian._build_resolver(["a/Dup.md", "b/Dup.md"])
        self.assertIsNone(memdag_obsidian._resolve_target("Dup", by_name, by_rel))

    def test_path_qualified_resolves_ambiguous(self):
        by_name, by_rel = memdag_obsidian._build_resolver(["a/Dup.md", "b/Dup.md"])
        self.assertEqual(
            memdag_obsidian._resolve_target("a/Dup", by_name, by_rel), "a/Dup.md")

    def test_missing_unresolved(self):
        by_name, by_rel = memdag_obsidian._build_resolver(["Note.md"])
        self.assertIsNone(memdag_obsidian._resolve_target("Ghost", by_name, by_rel))
...
class TestSync(Base):
    def test_ingest_creates_nodes_and_edges(self):
        ...
        summary = memdag_obsidian.sync_vault(self.conn, self.vault)
        self.assertEqual(summary["ingested"], 2)
        a = memdag_obsidian._representative_node(self.conn, "A.md")
        b = memdag_obsidian._representative_node(self.conn, "B.md")
        self.assertIsNotNone(a)
        self.assertIsNotNone(b)
        nb = memdag_relate.neighborhood(self.conn, a, hops=1)
    ...
    def test_unchanged_note_skipped_on_resync(self):
        ...
        s1 = memdag_obsidian.sync_vault(self.conn, self.vault)
        self.assertEqual(s1["ingested"], 1)
        s2 = memdag_obsidian.sync_vault(self.conn, self.vault)
    ...
    def test_edit_tombstones_old_and_reingests(self):
        ...
        memdag_obsidian.sync_vault(self.conn, self.vault)
        ...
        for nid in old:  # old nodes are tombstoned
            self.assertEqual(memdag.get_node(self.conn, nid)["tombstoned"], 1)
    ...
    def test_no_prune_keeps_deleted(self):
        ...
        memdag_obsidian.sync_vault(self.conn, self.vault)
        p.unlink()
        memdag_obsidian.sync_vault(self.conn, self.vault, prune=False)
    ...
    def test_mtime_preserving_content_swap_is_reingested(self):
        ...
        node = memdag.get_node(self.conn, min(new))
        self.assertIn("poisoned", node["content"])

    def test_oversized_note_skipped(self):
        self.note("Big.md", "x")
        big = self.vault / "Big.md"
        big.write_text("y" * (memdag_obsidian.MAX_NOTE_BYTES + 10), encoding="utf-8")
        summary = memdag_obsidian.sync_vault(self.conn, self.vault)
    ...
    def test_transient_missing_then_present_not_pruned(self):
        ...
        memdag_obsidian.sync_vault(self.conn, self.vault, prune=True)
...
class TestLaunderingLoop(Base):
    def test_memdag_authored_note_reenters_as_agent_derived(self):
        # 1. Seed a user fact and export an answer back to the vault.
        with self.conn:
            nid = memdag.insert_node(self.conn, "Postgres uses MVCC for concurrency.", "user")
        out = memdag_obsidian.export_note(self.conn, self.vault, node_id=nid)
        self.assertTrue(out.exists())
        fm, _ = memdag_obsidian.parse_frontmatter(out.read_text(encoding="utf-8"))
        self.assertEqual(fm["memdag-channel"], "agent-derived")
        self.assertEqual(str(fm["memdag-authored"]).lower(), "true")
        ...
        for n in nodes:
            self.assertEqual(self.channel_of(n), "agent-derived",
                             "memdag-authored note laundered up to user!")
...
class TestExport(Base):
    def test_export_requires_exactly_one_source(self):
        with self.assertRaises(ValueError):
            memdag_obsidian.export_note(self.conn, self.vault)
        with self.assertRaises(ValueError):
            memdag_obsidian.export_note(self.conn, self.vault, node_id=1, query="x")

    def test_refuses_to_overwrite_foreign_note(self):
        with self.conn:
            nid = memdag.insert_node(self.conn, "some content", "user")
        # Pre-create a NON-memdag note where the export would land.
        title = f"memdag node {nid}"
        foreign = self.vault / "memdag" / f"{memdag_obsidian._slugify(title)}.md"
        foreign.parent.mkdir(parents=True, exist_ok=True)
        foreign.write_text("# Hand-written, precious\n", encoding="utf-8")
        with self.assertRaises(ValueError):
            memdag_obsidian.export_note(self.conn, self.vault, node_id=nid)
        # The user's content is untouched.
        self.assertIn("precious", foreign.read_text(encoding="utf-8"))

    def test_overwrites_own_note(self):
        with self.conn:
            nid = memdag.insert_node(self.conn, "content v1", "user")
        p1 = memdag_obsidian.export_note(self.conn, self.vault, node_id=nid)
        # Second export to the same title must succeed (memdag authored it).
        p2 = memdag_obsidian.export_note(self.conn, self.vault, node_id=nid)
    ...
    def test_query_export_is_clean_and_captures_sources(self):
        ...
        out = memdag_obsidian.export_note(
            self.conn, self.vault, query="Tell me about the Redis store")
        fm, body = memdag_obsidian.parse_frontmatter(out.read_text(encoding="utf-8"))
        ...
        self.assertNotEqual(fm[memdag_obsidian.SOURCES_KEY], [])  # real source captured

    def test_authored_flag_alone_does_not_grant_overwrite(self):
        # re-audit (low): a hand note carrying only `memdag-authored: true` (but
        # NOT the source-nodes key memdag always writes) must NOT be overwritable.
        with self.conn:
            nid = memdag.insert_node(self.conn, "content", "user")
        title = f"memdag node {nid}"
        foreign = self.vault / "memdag" / f"{memdag_obsidian._slugify(title)}.md"
        foreign.parent.mkdir(parents=True, exist_ok=True)
        foreign.write_text("---\nmemdag-authored: true\n---\nhand-written\n", encoding="utf-8")
        with self.assertRaises(ValueError):
            memdag_obsidian.export_note(self.conn, self.vault, node_id=nid)
...
class TestWatcher(Base):
    def test_half_write_then_complete_syncs_final_content(self):
        ...
        memdag_obsidian.watch_vault(
            memdag.get_connection, self.vault, "user",
    ...
    def test_snapshot_uses_mtime_ns_int(self):
        self.note("X.md", "x")
        snap = memdag_obsidian._snapshot(self.vault)
...
class TestCli(Base):
    def test_sync_via_cli(self):
        ...
        self.note("B.md", "beta")
        memdag_cli.main(["obsidian-sync", str(self.vault)])

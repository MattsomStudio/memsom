#!/usr/bin/env python3
"""Tests for memdag_profile — leaf-origin PROFILE (display-only).

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_profile.py \
...
class TestChainHistogram(Base):
    ...
    def test_chain_histogram(self):
        ...
        a, _ = memdag.derive_node(self.conn, "a", [e, u, x])
        b, _ = memdag.derive_node(self.conn, "b", [a])

        p = memdag_profile.profile(self.conn, b)
...
class TestDiamondLeafCountedOnce(Base):
    ...
    def test_diamond_leaf_counted_once(self):
        ...
        l, _ = memdag.derive_node(self.conn, "left", [s])
        r, _ = memdag.derive_node(self.conn, "right", [s])
        j, _ = memdag.derive_node(self.conn, "join", [l, r])

        p = memdag_profile.profile(self.conn, j)
...
class TestFloorEqualsGetNodeLabel(Base):
    ...
    def test_floor_equals_get_node_label(self):
        ...
        for nid in (e, u, x, a, b):
            p = memdag_profile.profile(self.conn, nid)
            stored_label = memdag.get_node(self.conn, nid)["label"]
            self.assertEqual(
...
class TestExcludesTombstonedAndRedactedLeaves(Base):
    ...
    def test_excludes_tombstoned_and_redacted_leaves(self):
        ...
        d, _ = memdag.derive_node(self.conn, "derived", [s1, s2, s3])

        # Add the redacted column
        memdag_schema.add_column(self.conn, "nodes", "redacted", "INTEGER NOT NULL DEFAULT 0")
        ...
        p = memdag_profile.profile(self.conn, d)
...
class TestPaddingChangesHistogramNeverFloor(Base):
    ...
    def test_padding_changes_histogram_never_floor(self):
        ...
        d1, _ = memdag.derive_node(self.conn, "d1", [x, e1])
        ...
        d2, _ = memdag.derive_node(self.conn, "d2", [x, e1, e2, e3, e4, e5])

        p1 = memdag_profile.profile(self.conn, d1)
        p2 = memdag_profile.profile(self.conn, d2)
...
class TestBareSourceIsItsOwnLeaf(Base):
    ...
    def test_external_source_is_own_leaf(self):
        x = self.add("external content", "external")
        p = memdag_profile.profile(self.conn, x)
    ...
    def test_endorsed_source_no_external_leaf(self):
        e = self.add("endorsed content", "endorsed")
        p = memdag_profile.profile(self.conn, e)
...
class TestUnknownIdRaises(Base):
    ...
    def test_unknown_id_raises(self):
        with self.assertRaises(ValueError):
            memdag_profile.profile(self.conn, 999)
...
class TestFormatProfileExact(Base):
    ...
    def test_format_without_external(self):
        e = self.add("endorsed content", "endorsed")
        u = self.add("user content", "user")
        a, _ = memdag.derive_node(self.conn, "a", [e, u])

        p = memdag_profile.profile(self.conn, a)
        # No external suffix
...
class TestProfileNeverWrites(Base):
    ...
    def test_profile_never_writes(self):
        e = self.add("endorsed content", "endorsed")
        x = self.add("external content", "external")
        d, _ = memdag.derive_node(self.conn, "derived", [e, x])
        ...
        memdag_profile.profile(self.conn, d)
...
class TestCliProfilePrints(Base):
    ...
    def test_cli_prints_floor(self):
        ...
        x = self.add("external content", "external")
        a, _ = memdag.derive_node(self.conn, "a", [e, u, x])
        j, _ = memdag.derive_node(self.conn, "b", [a])

        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            memdag_profile.main(["profile", str(j)])
    ...
    def test_cli_unknown_id_exits_1(self):
        with self.assertRaises(SystemExit) as cm:
            memdag_profile.main(["profile", "999"])

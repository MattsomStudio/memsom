#!/usr/bin/env python3
"""Tests for memdag_rederive — run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memdag_rederive.py \
    -t <repo> -v
"""
...
class TestMigrateIdempotent(Base):
    ...
    def test_migrate_twice(self):
        memdag_rederive.migrate(self.conn)  # second call (setUp ran the first)
...
class TestRecipeCapture(Base):
    ...
    def test_compose_recipe_roundtrip(self):
        ...
        c, _ = memdag.derive_node(self.conn, "derived from a", [a])
        with self.conn:
            memdag_rederive.record_recipe(self.conn, c, "compose", question="what is a?")

        got = memdag_rederive.get_recipe(self.conn, c)
    ...
    def test_extractive_recipe_is_null_blob(self):
        ...
        c, _ = memdag.derive_node(self.conn, "derived from a", [a])
        with self.conn:
            memdag_rederive.record_recipe(self.conn, c, "extractive")
    ...
    def test_unrecorded_node_returns_none(self):
        a = self.add("endorsed source content text here", "endorsed")
        c, _ = memdag.derive_node(self.conn, "derived from a", [a])
        self.assertIsNone(memdag_rederive.get_recipe(self.conn, c))  # legacy = None = 'unknown'

    def test_replace_on_conflict(self):
        a = self.add("endorsed source content text here", "endorsed")
        c, _ = memdag.derive_node(self.conn, "derived from a", [a])
        with self.conn:
            memdag_rederive.record_recipe(self.conn, c, "llm", model="x")
            memdag_rederive.record_recipe(self.conn, c, "compose", question="q")  # INSERT OR REPLACE
        got = memdag_rederive.get_recipe(self.conn, c)
        self.assertEqual(got["engine"], "compose")
...
class RegenBase(Base):
    ...
    def make_compose(self, question, parent_ids):
        ...
        with self.conn:
            c, _ = memdag.derive_node(self.conn, text, used)
            memdag_rederive.record_recipe(self.conn, c, "compose", question=question)
        return c, text
    ...
    def make_extractive(self, parent_ids, k=5):
        ...
        rows = sorted(((i, self.conn.execute(
            ...
            key=lambda r: r[0])
        text = memdag_compact.extractive_summary(rows, k)
        with self.conn:
            c, _ = memdag.derive_node(self.conn, text, parent_ids)
            memdag_rederive.record_recipe(self.conn, c, "extractive", k=k)
        return c, text
...
class TestRegenerateCompose(RegenBase):
    ...
    def test_changed_mints_and_archives(self):
        ...
        self.kill(b)
        new_id = memdag_rederive.regenerate(self.conn, c)
        ...
        exp_text, _ = memdag.compose("apples", self.src_rows([a]))
        ...
        rec = memdag_rederive.get_recipe(self.conn, new_id)
    ...
    def test_no_churn_when_nothing_changed(self):
        ...
        before = self.n_nodes()
        self.assertIsNone(memdag_rederive.regenerate(self.conn, c))  # no parent died
    ...
    def test_all_parents_gone_retire_no_replace(self):
        ...
        self.kill(b)
        before = self.n_nodes()
        self.assertIsNone(memdag_rederive.regenerate(self.conn, c))
    ...
    def test_idempotent_retry(self):
        ...
        first = memdag_rederive.regenerate(self.conn, c)
        n_after_first = self.n_nodes()
        self.assertIsNone(memdag_rederive.regenerate(self.conn, c))     # c now archived -> no-op
        self.assertIsNone(memdag_rederive.regenerate(self.conn, first)) # fresh -> unchanged -> no-op
...
class TestRegenerateGuards(RegenBase):
    ...
    def test_llm_engine_not_rolled(self):
        ...
        b = self.add("Apples can be pressed into cider during the autumn apple harvest.", "endorsed")
        with self.conn:
            c, _ = memdag.derive_node(self.conn, "llm prose summary", [a, b])
            memdag_rederive.record_recipe(self.conn, c, "llm", model="x", prompt="p")
        self.kill(b)
        self.assertIsNone(memdag_rederive.regenerate(self.conn, c))
    ...
    def test_unknown_legacy_node_not_rolled(self):
        a = self.add("Apples are red fruits that grow on apple trees in orchards.", "endorsed")
        with self.conn:
            c, _ = memdag.derive_node(self.conn, "legacy derived", [a])  # no recipe recorded
        self.kill(a)
        self.assertIsNone(memdag_rederive.regenerate(self.conn, c))
...
class TestRegenerateExtractive(RegenBase):
    ...
    def test_extractive_regenerates(self):
        ...
        exp = memdag_compact.extractive_summary(
...
class TestErase(RegenBase):
    ...
    def test_erase_source_scrubs_derived_keeps_edges(self):
        ...
        erased = memdag_rederive.erase(self.conn, a, "PII scrub")
    ...
    def test_erase_follows_supersedes_chain(self):
        ...
        c, _ = self.make_compose("apples", [a, b])
        self.kill(b)
        cprime = memdag_rederive.regenerate(self.conn, c)   # c archived, cprime.supersedes = c
        self.assertIsNotNone(cprime)
        self.assertEqual(self.archived(c), 1)

        erased = memdag_rederive.erase(self.conn, cprime, "scrub the secret")
    ...
    def test_erase_is_idempotent(self):
        ...
        first = memdag_rederive.erase(self.conn, a, "scrub")
        second = memdag_rederive.erase(self.conn, a, "scrub again")  # already-redacted skipped

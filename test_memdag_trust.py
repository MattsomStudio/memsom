#!/usr/bin/env python3
"""Tests for memdag_trust — trust algebra: integrity lattice + audited elevation.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s C:\\Users\\you\\memdag -p test_memdag_trust.py \
...
class Base(unittest.TestCase):
    """Temp-DB base class — mirrors test_memdag.py Base exactly."""
    ...
    def setUp(self):
        ...
        memdag_trust.migrate(self.conn)
...
class TestAuditRowWritten(Base):
    def test_audit_row_written(self):
        ...
        result = memdag_trust.elevate(self.conn, x, "user", "verified source", "matt")
        ...
        node = memdag.get_node(self.conn, x)
...
class TestDescendantsRefloored(Base):
    def test_descendants_refloor_upward(self):
        ...
        d1, _ = memdag.derive_node(self.conn, "d1 text", [x])  # label 0
        d2, _ = memdag.derive_node(self.conn, "d2 text", [d1]) # label 0

        result = memdag_trust.elevate(self.conn, x, "user", "trust elevation", "matt")
        ...
        self.assertEqual(memdag.get_node(self.conn, d1)["label"], 2)
...
class TestElevatedDerivedNodeFixedPoint(Base):
    def test_elevated_derived_node_not_clawed_back(self):
        x = self.add("external source", "external")        # label 0
        d, _ = memdag.derive_node(self.conn, "derived", [x])  # label 0

        # Elevate the derived node itself (from 0 to 2)
        memdag_trust.elevate(self.conn, d, "user", "manual trust", "matt")
...
class TestExternalToEndorsedPolicy(Base):
    def test_external_to_endorsed_blocked_then_forced(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            memdag_trust.elevate(self.conn, x, "endorsed", "jump attempt", "matt")
        self.assertIn("blocked", str(ctx.exception))

        # Label unchanged
        self.assertEqual(memdag.get_node(self.conn, x)["label"], 0)
        # No audit row written on the failed attempt
        self.assertEqual(len(memdag_trust.elevations_for(self.conn, x)), 0)

        # With force: must succeed and record forced=1
        result = memdag_trust.elevate(self.conn, x, "endorsed", "forced jump", "matt",
                                      force=True)
...
class TestLatticeLaws(Base):
    def test_lattice_laws(self):
        ...
        for a in vals:
            # Idempotent
            self.assertEqual(memdag_trust.meet(a, a), a, f"meet idempotent fail a={a}")
            self.assertEqual(memdag_trust.join(a, a), a, f"join idempotent fail a={a}")

            for b in vals:
                # Commutative
                self.assertEqual(memdag_trust.meet(a, b), memdag_trust.meet(b, a),
                                 f"meet commutative fail a={a} b={b}")
                self.assertEqual(memdag_trust.join(a, b), memdag_trust.join(b, a),
                                 f"join commutative fail a={a} b={b}")

                # Absorption: join(a, meet(a, b)) == a
                self.assertEqual(memdag_trust.join(a, memdag_trust.meet(a, b)), a,
                                 f"absorption fail a={a} b={b}")

                for c in vals:
                    # Associative
                    self.assertEqual(
                        memdag_trust.meet(a, memdag_trust.meet(b, c)),
                        memdag_trust.meet(memdag_trust.meet(a, b), c),
                        f"meet associative fail a={a} b={b} c={c}"
                    )
                    self.assertEqual(
                        memdag_trust.join(a, memdag_trust.join(b, c)),
                        memdag_trust.join(memdag_trust.join(a, b), c),
...
class TestValidation(Base):
    def test_unknown_id(self):
        with self.assertRaises(ValueError):
            memdag_trust.elevate(self.conn, 9999, "user", "r", "matt")

    def test_tombstoned_node(self):
        x = self.add("data", "external")
        memdag.revoke_cascade(self.conn, x, "revoked")
        with self.assertRaises(ValueError):
            memdag_trust.elevate(self.conn, x, "user", "r", "matt")
    ...
    def test_downward_elevation(self):
        x = self.add("user data", "user")   # label 2
        with self.assertRaises(ValueError) as ctx:
            memdag_trust.elevate(self.conn, x, "external", "r", "matt")
        self.assertIn("lowering", str(ctx.exception))

    def test_bad_label_name(self):
        x = self.add("data", "external")
        with self.assertRaises(ValueError):
            memdag_trust.elevate(self.conn, x, "superduper", "r", "matt")

    def test_meet_out_of_range(self):
        with self.assertRaises(ValueError):
            memdag_trust.meet(0, 4)
        with self.assertRaises(ValueError):
            memdag_trust.meet(-1, 0)

    def test_join_out_of_range(self):
        with self.assertRaises(ValueError):
            memdag_trust.join(3, 5)
        with self.assertRaises(ValueError):
            memdag_trust.join(0, -1)

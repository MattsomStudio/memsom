#!/usr/bin/env python3
"""Tests for memsom_broker — Gate #3 MCP broker (config, pins, gate forward).

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_broker.py -t <repo> -v
"""
...
class TestDecideAndForward(Base):
    ...
    def test_fetch_taints_then_send_denied_and_not_forwarded(self):
        ...
        self.assertEqual(memsom_session.current_floor(self.conn, self.sid),
                         memsom.RANK["external"])
    ...
    def test_read_tool_never_denied_even_when_tainted(self):
        memsom_session.lower_floor(self.conn, self.sid, "external", "x", "test")
        r = B.decide_and_forward(self.conn, self.cfg, self.policy, self.sid,
                                 "fetch.fetch", {}, self.caller)
        self.assertFalse(r["isError"])
    ...
    def test_aggregated_tools_namespaced(self):
        ...
        self.assertTrue(any(n.startswith("memsom.") for n in names))

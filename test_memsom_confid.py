#!/usr/bin/env python3
"""Tests for memsom_confid — Bell-LaPadula confidentiality axis.

Run:
    python -W error::DeprecationWarning -m unittest discover \
        -s <repo> -p test_memsom_confid.py \
        -t <repo> -v
"""
...
class Base(unittest.TestCase):
    """Temp-DB base — identical pattern to test_memsom.py Base."""
    ...
    def setUp(self):
        ...
        memsom_confid.migrate(self.conn)   # add conf_label column
    ...
    def get_label(self, nid):
        ...
        return memsom.get_node(self.conn, nid)["label"]
...
class TestDerivedConfIsMaxNotMin(Base):
    ...
    def test_derived_conf_is_max_not_min(self):
        ...
        memsom_confid.classify(self.conn, p, "public")   # conf 0
        memsom_confid.classify(self.conn, s, "secret")   # conf 2
        ...
        self.assertEqual(self.get_label(d), min(memsom.RANK["endorsed"], memsom.RANK["user"]))
...
class TestAxesIndependent(Base):
    ...
    def test_axes_independent(self):
        ...
        memsom_confid.classify(self.conn, ext, "topsecret")
        ...
        # Derive from both
        d, d_label = memsom.derive_node(self.conn, "derived answer text", [ext, end])
...
class TestSourcesUntouched(Base):
    ...
    def test_sources_untouched(self):
        ...
        memsom_confid.classify(self.conn, src, "secret")  # conf 2

        old, new = memsom_confid.recompute_conf(self.conn, src)
        ...
        d, _ = memsom.derive_node(self.conn, "derived node answer text", [src])
        # Classify derived node manually to a wrong value so recompute_conf_all has something to fix
        memsom_confid.classify(self.conn, d, "public")
...
class TestParseConfValidation(Base):
    ...
    def test_parse_conf_validation(self):
        ...
        self.assertEqual(memsom_confid.parse_conf("SECRET"), 2)
        self.assertEqual(memsom_confid.parse_conf("secret"), 2)
        self.assertEqual(memsom_confid.parse_conf(2), 2)

        with self.assertRaises(ValueError):
            memsom_confid.parse_conf("ultra")
        with self.assertRaises(ValueError):
            memsom_confid.parse_conf(5)
...
class TestTombstonedParentExcluded(Base):
    ...
    def test_tombstoned_parent_excluded(self):
        ...
        memsom_confid.classify(self.conn, p, "public")   # 0
        memsom_confid.classify(self.conn, s, "secret")   # 2

        d, _ = memsom.derive_node(self.conn, "derived answer node", [p, s])
        ...
        old2, new2 = memsom_confid.recompute_conf(self.conn, d)
...
class TestCli(Base):
    ...
    def test_cli_classify_and_recompute(self):
        ...
        # Derived node
        d, _ = memsom.derive_node(self.conn, "derived answer text", [n])
    ...
    def test_cli_recompute_unchanged(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived text answer node", [n])
    ...
    def test_cli_recompute_all_nothing_changed(self):
        n = self.add("public source node text", "user")
        memsom.derive_node(self.conn, "derived answer node text", [n])
        out = self.run_main("conf-recompute", "--all")
...
class TestArchivedConfLaunderingBlocked(Base):
    ...
    def test_classify_archived_parents_cannot_launder_summary(self):
        ...
        for sid in (s1, s2):
            ...
            with self.assertRaises(ValueError):
                memsom_confid.classify(self.conn, sid, "public")
    ...
    def test_archived_node_can_still_be_raised(self):
        ...
        s = memsom.insert_node(self.conn, "internal alpha note one two three", "user")
        memsom_confid.classify(self.conn, s, "internal")  # 1
        memsom.insert_node(self.conn, "internal alpha note one two four", "user")
        ...
        for extra in self.conn.execute(
            ...
            memsom_confid.classify(self.conn, extra[0], "internal")
        memsom_compact.compact(self.conn, group_by="similarity",
                               min_group=2, sim_threshold=0.0)
        # s is archived now; raising it to secret is fine
        memsom_confid.classify(self.conn, s, "secret")
    ...
    def test_retrieve_public_never_surfaces_secret_after_launder_attempt(self):
        ...
        minted = memsom_compact.compact(
            self.conn, group_by="similarity", min_group=2, sim_threshold=0.0)
        nid = minted[0]
        memsom_retrieve.index_node(self.conn, s1)
        memsom_retrieve.index_node(self.conn, s2)
...
class TestRecomputeConfOrderIndependent(Base):
    ...
    def _out_of_order_dag(self):
        ...
        """
        with self.conn:
            dc = memsom.insert_node(self.conn, "child", "agent-derived", 0)   # id 1
        with self.conn:
            s = memsom.insert_node(self.conn, "secret source", "endorsed", 3)  # id 2
        with self.conn:
            dp = memsom.insert_node(self.conn, "mid", "agent-derived", 0)      # id 3
        with self.conn:
    ...
    def test_high_water_propagates_despite_low_child_id(self):
        ...
        memsom_confid.recompute_conf_all(self.conn)
    ...
    def test_idempotent_on_correct_db(self):
        ...
        memsom_confid.recompute_conf_all(self.conn)         # converge
        again = memsom_confid.recompute_conf_all(self.conn)  # CONFID-2

#!/usr/bin/env python3
"""Regression tests for memsom_federation security hardening (federation fix key).
...
Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_federation_security.py \
    -t <repo> -v
"""
...
class SecurityBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.conn = memsom.get_connection(self.db_path)
    ...
    def _local_endorsed(self, content="Trusted endorsed fact.", origin="local"):
        ...
        with self.conn:
            nid = memsom.insert_node(self.conn, content, "endorsed")
        memsom_federation.backfill_uuids(self.conn, origin)
...
class TestUntrustedChannelClamped(SecurityBase):
    ...
    def test_untrusted_channel_clamped_to_external(self):
        ...
        stats = memsom_federation.import_changeset(self.conn, cs)
        ...
        result = memsom_gate.check_action(self.conn, nid, "endorsed")
...
class TestForgedEdgeOntoLocalRejected(SecurityBase):
    ...
    def test_forged_edge_onto_local_rejected(self):
        ...
        memsom_federation.register_origin(self.conn, "local", by="test")
        ...
        parents = memsom.parents_of(self.conn, endorsed_id)
...
class TestForgedAgentLabelRecomputed(SecurityBase):
    ...
    def test_forged_agent_label_recomputed(self):
        ...
        # Gate must deny endorsed action
        memsom_gate.migrate(self.conn)
        result = memsom_gate.check_action(self.conn, child_id, "endorsed")
        self.assertEqual(result["decision"], "deny",
...
class TestMaliciousTombstoneIgnored(SecurityBase):
    ...
    def test_malicious_tombstone_ignored(self):
        ...
        memsom_federation.register_origin(self.conn, victim_origin, by="test")
...
class TestRedactedImportStripsContent(SecurityBase):
    ...
    def test_redacted_import_strips_content(self):
        ...
        sources = memsom.live_sources(self.conn)
        compose_result, _ = memsom.compose("admin password reset token", sources)
        if compose_result:
...
class TestOldChangesetCannotRestoreRedacted(SecurityBase):
    ...
    def test_old_changeset_cannot_restore_redacted(self):
        ...
        src_origin = "machineA"
        memsom_federation.register_origin(self.conn, src_origin, by="test")

        # Insert a node and backfill UUID
        with self.conn:
            src_id = memsom.insert_node(self.conn, SECRET, "user")
        memsom_federation.backfill_uuids(self.conn, src_origin)
        ...
        with self.conn:
            self.conn.execute(
                ...
                " WHERE id=?",
                (memsom.now_iso(), "test-redaction", src_id)
            )
        ...
        stats = memsom_federation.import_changeset(self.conn, old_cs)
...
class TestConfUnderclassificationCorrected(SecurityBase):
    ...
    def test_conf_underclassification_corrected(self):
        ...
        self.assertGreaterEqual(
            stored_conf,
            memsom_federation.CONF_FLOOR_UNTRUSTED,
            f"untrusted conf_label=0 must be floored to >= {memsom_federation.CONF_FLOOR_UNTRUSTED}, got {stored_conf}"
        )
...
class TestRegisterOriginAndList(SecurityBase):
    ...
    def test_register_origin_and_list(self):
        ...
        self.assertFalse(
            memsom_federation.is_trusted(self.conn, "peer"),
            "unregistered origin must not be trusted"
        )

        # Register the origin
        result = memsom_federation.register_origin(self.conn, "peer", by="test",
                                                   descr="test peer")
        ...
        origins = [row[0] for row in memsom_federation.list_origins(self.conn)]
        self.assertIn("peer", origins, "list_origins must include the registered peer")

        # None and empty string are never trusted
        self.assertFalse(memsom_federation.is_trusted(self.conn, None),
                         "None origin must not be trusted")
        self.assertFalse(memsom_federation.is_trusted(self.conn, ""),
                         "empty string origin must not be trusted")

        # Idempotent: re-registering does not raise and still returns True
        result2 = memsom_federation.register_origin(self.conn, "peer", by="test2")
...
class TestTombstoneCascadeTriggeredOnImport(SecurityBase):
    ...
    def test_tombstone_cascade_triggered_on_import(self):
        ...
        with self.conn:
            src_id = memsom.insert_node(self.conn, "Source endorsed content.", "endorsed")
            child1_id, _ = memsom.derive_node(self.conn, "Derived from endorsed.", [src_id])
            child2_id, _ = memsom.derive_node(self.conn, "Derived from child1.", [child1_id])
...
class TestTrustedOriginRoundTrip(SecurityBase):
    ...
    def test_trusted_origin_round_trip_unchanged(self):
        ...
        memsom_federation.register_origin(self.conn, trusted_origin, by="test")
...
class TestFed2RedactionCascadeOnImport(SecurityBase):
    ...
    def test_redaction_cascades_to_descendant(self):
        origin = "peer"
        memsom_federation.register_origin(self.conn, origin, by="test")

        with self.conn:
            x = memsom.insert_node(self.conn, "secret root fact", "endorsed")
        d, _ = memsom.derive_node(self.conn, "child quoting the secret root fact", [x])

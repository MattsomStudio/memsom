#!/usr/bin/env python3
"""Tests for memsom_ingest.

Run:
  python -W error::DeprecationWarning -m unittest discover \
      -s <repo> -p test_memsom_ingest.py \
      -t <repo> -v
"""
...
class TestDedupSkipsTaintedNodes(Base):
    def test_dedup_does_not_reuse_redacted_node(self):
        ...
        memsom_redact.redact_node(self.conn, nid, "scrub", cascade=True)
        self.assertEqual(
            self.conn.execute("SELECT content FROM nodes WHERE id=?", (nid,)).fetchone()[0],
            "", "redacted node content must be empty")

        again = memsom_ingest.ingest_text(self.conn, text, "user", chunk=False)
    ...
    def test_dedup_is_channel_aware(self):
        ...
        u = memsom_ingest.ingest_text(self.conn, text, "user", chunk=False)[0]
        a = memsom_ingest.ingest_text(self.conn, text, "agent-derived", chunk=False)[0]
        ...
        u2 = memsom_ingest.ingest_text(self.conn, text, "user", chunk=False)[0]
    ...
    def test_dedup_does_not_reuse_quarantined_node(self):
        ...
        again = memsom_ingest.ingest_text(self.conn, text, "user", chunk=False)[0]
...
class TestMigration(Base):
    def test_content_hash_column_exists_after_migrate(self):
        self.assertTrue(
            memsom_schema.column_exists(self.conn, "nodes", "content_hash"),
    ...
    def test_migrate_idempotent(self):
        ...
        memsom_ingest.migrate(self.conn)
        self.assertTrue(memsom_schema.column_exists(self.conn, "nodes", "content_hash"))
    ...
    def test_existing_nodes_have_null_content_hash(self):
        ...
        conn2 = memsom.get_connection()
        try:
            nid = memsom.insert_node(conn2, "some content", "user")
            # NOW migrate
            memsom_ingest.migrate(conn2)
            row = conn2.execute(
...
class TestIngestTextBasic(Base):
    def test_creates_node_at_declared_channel_endorsed(self):
        ids = memsom_ingest.ingest_text(self.conn, "Hello endorsed world.", "endorsed")
        self.assertEqual(len(ids), 1)
        node = memsom.get_node(self.conn, ids[0])
        self.assertEqual(node["channel"], "endorsed")
        self.assertEqual(node["label"], memsom.RANK["endorsed"])

    def test_creates_node_at_declared_channel_external(self):
        ids = memsom_ingest.ingest_text(self.conn, "External article content.", "external")
        self.assertEqual(len(ids), 1)
        node = memsom.get_node(self.conn, ids[0])
        self.assertEqual(node["channel"], "external")
        self.assertEqual(node["label"], memsom.RANK["external"])

    def test_creates_node_at_declared_channel_user(self):
        ids = memsom_ingest.ingest_text(self.conn, "User stated fact.", "user")
        self.assertEqual(len(ids), 1)
        node = memsom.get_node(self.conn, ids[0])
        self.assertEqual(node["channel"], "user")

    def test_content_hash_set_on_inserted_node(self):
        ids = memsom_ingest.ingest_text(self.conn, "Content with a hash.", "user")
    ...
    def test_empty_text_returns_empty_list(self):
        ids = memsom_ingest.ingest_text(self.conn, "", "user")
        self.assertEqual(ids, [])

    def test_whitespace_only_returns_empty_list(self):
        ids = memsom_ingest.ingest_text(self.conn, "   \n\n   ", "user")
        self.assertEqual(ids, [])
...
class TestIngestTextDedup(Base):
    def test_identical_content_dedups_to_same_id(self):
        ...
        ids1 = memsom_ingest.ingest_text(self.conn, text, "user")
        ids2 = memsom_ingest.ingest_text(self.conn, text, "user")
    ...
    def test_whitespace_normalized_for_dedup(self):
        ...
        ids_a = memsom_ingest.ingest_text(self.conn, text_a, "user")
        ids_b = memsom_ingest.ingest_text(self.conn, text_b, "user")
        self.assertEqual(ids_a[0], ids_b[0], "Whitespace normalization should dedup")

    def test_different_content_gets_different_ids(self):
        ids1 = memsom_ingest.ingest_text(self.conn, "Alpha content for test.", "user")
        ids2 = memsom_ingest.ingest_text(self.conn, "Beta content for test.", "user")
    ...
    def test_tombstoned_node_not_reused_for_dedup(self):
        ...
        nid1 = ids1[0]
        # Tombstone the node
        memsom.revoke_cascade(self.conn, nid1, "test revoke")
...
class TestIngestTextChunking(Base):
    ...
    def test_long_doc_creates_multiple_chunks(self):
        ...
        ids = memsom_ingest.ingest_text(self.conn, doc, "user", chunk_chars=1200)
    ...
    def test_chunk_nodes_have_chunk_ref_in_source_ref(self):
        ...
        for idx, nid in enumerate(ids):
            node = memsom.get_node(self.conn, nid)
            self.assertIn(
    ...
    def test_chunking_disabled_stores_single_node(self):
        doc = self._make_long_doc()
        ids = memsom_ingest.ingest_text(self.conn, doc, "user", chunk=False)
    ...
    def test_short_doc_not_chunked(self):
        ...
        ids = memsom_ingest.ingest_text(self.conn, text, "user", chunk_chars=1200)
        self.assertEqual(len(ids), 1)
    ...
    def test_chunk_dedup_within_same_doc(self):
        ...
        ids = memsom_ingest.ingest_text(self.conn, doc, "user", chunk_chars=200)
    ...
    def test_all_content_covered(self):
        ...
        ids = memsom_ingest.ingest_text(self.conn, doc, "user", chunk_chars=500)
        seen_ids = set(ids)
        all_content = ""
        for nid in seen_ids:
            node = memsom.get_node(self.conn, nid)
...
class TestIngestFile(Base):
    def test_ingest_file_creates_node(self):
        ...
        ids = memsom_ingest.ingest_file(self.conn, f, "endorsed")
    ...
    def test_ingest_file_source_ref_is_path(self):
        ...
        ids = memsom_ingest.ingest_file(self.conn, f, "user")
        node = memsom.get_node(self.conn, ids[0])
...
class TestIngestDir(Base):
    ...
    def test_ingest_dir_finds_md_files(self):
        ...
        ids = memsom_ingest.ingest_dir(self.conn, base, "endorsed")
        # 3 .md files, each short enough for 1 node
        self.assertEqual(len(ids), 3)

    def test_ingest_dir_ignores_non_glob_files(self):
        base = self._make_dir_tree()
        ids = memsom_ingest.ingest_dir(self.conn, base, "endorsed", glob="*.md")
    ...
    def test_ingest_dir_custom_glob(self):
        base = self._make_dir_tree()
        ids = memsom_ingest.ingest_dir(self.conn, base, "user", glob="*.txt")
    ...
    def test_ingest_dir_channel_stamped_correctly(self):
        base = self._make_dir_tree()
        ids = memsom_ingest.ingest_dir(self.conn, base, "external")
        for nid in ids:
            node = memsom.get_node(self.conn, nid)
            self.assertEqual(node["channel"], "external")
    ...
    def test_ingest_dir_empty_dir_returns_empty_list(self):
        ...
        empty.mkdir()
        ids = memsom_ingest.ingest_dir(self.conn, empty, "user")
        self.assertEqual(ids, [])
...
class TestIngestUrl(Base):
    ...
    def test_ingest_url_channel_forced_external(self):
        ...
        with patch("urllib.request.urlopen", mock_open):
            ids = memsom_ingest.ingest_url(self.conn, "https://example.com/doc")
        self.assertGreater(len(ids), 0)
        node = memsom.get_node(self.conn, ids[0])
        self.assertEqual(node["channel"], "external",
    ...
    def test_ingest_url_source_ref_is_url(self):
        ...
        mock_open = self._make_mock_urlopen(body)
        with patch("urllib.request.urlopen", mock_open):
            ids = memsom_ingest.ingest_url(self.conn, url)
        node = memsom.get_node(self.conn, ids[0])
    ...
    def test_ingest_url_user_agent_set(self):
        ...
        with patch("urllib.request.urlopen", mock_open):
            memsom_ingest.ingest_url(self.conn, "https://example.com/")
    ...
    def test_ingest_url_dedup_same_url_same_content(self):
        ...
        mock_open = self._make_mock_urlopen(body)
        with patch("urllib.request.urlopen", mock_open):
            ids1 = memsom_ingest.ingest_url(self.conn, "https://example.com/stable")
        with patch("urllib.request.urlopen", mock_open):
            ids2 = memsom_ingest.ingest_url(self.conn, "https://example.com/stable")
    ...
    def test_ingest_url_latin1_fallback(self):
        ...
        mock_open = self._make_mock_urlopen(body)
        with patch("urllib.request.urlopen", mock_open):
            ids = memsom_ingest.ingest_url(self.conn, "https://example.com/latin")
        self.assertGreater(len(ids), 0)
...
class TestChunkSplitter(unittest.TestCase):
    ...
    def test_short_text_single_chunk(self):
        text = "Short text."
        chunks = memsom_ingest._split_chunks(text, 1200)
    ...
    def test_paragraph_boundary_preferred(self):
        ...
        para2 = "B " * 300
        text = para1.rstrip() + "\n\n" + para2.rstrip()
        chunks = memsom_ingest._split_chunks(text, 700)
    ...
    def test_no_empty_chunks(self):
        ...
        chunks = memsom_ingest._split_chunks(text, 400)
        for ch in chunks:
    ...
    def test_all_content_preserved(self):
        ...
        chunks = memsom_ingest._split_chunks(text, 500)
...
class TestRegister(Base):
    ...
    def test_ingest_text_subcommand_functional(self):
        ...
        memsom_ingest._cmd_ingest_text(args)
...
class TestFrozenCoreCompat(Base):
    def test_insert_node_still_works_after_migrate(self):
        """memsom.insert_node must work normally after ingest migration."""
        nid = memsom.insert_node(self.conn, "post-migrate insert", "user")
        node = memsom.get_node(self.conn, nid)
        self.assertEqual(node["channel"], "user")

    def test_derive_node_still_works_after_migrate(self):
        a = self.add("parent content", "endorsed")
        b, label = memsom.derive_node(self.conn, "derived content", [a])
        self.assertEqual(label, memsom.RANK["endorsed"])
    ...
    def test_content_hash_null_on_direct_insert(self):
        ...
        nid = memsom.insert_node(self.conn, "direct insert", "user")
        row = self.conn.execute(
            "SELECT content_hash FROM nodes WHERE id=?", (nid,)
        ).fetchone()
...
class TestChannelLabelLock(Base):
    ...
    def test_authoritative_label_matches_channel(self):
        for ch, rank in memsom.RANK.items():
            self.assertEqual(memsom_ingest.authoritative_label(ch), rank)
        with self.assertRaises(ValueError):
            memsom_ingest.authoritative_label("superuser")
    ...
    def test_ingest_text_stamps_channel_label_not_caller_label(self):
        ...
        for ch in ("external", "user", "endorsed"):
            ...
            for nid in ids:
                ...
                self.assertEqual(node["label"], memsom.RANK[ch],
...
class TestChannelCeiling(Base):
    ...
    def tearDown(self):
        os.environ.pop(memsom_ingest.CHANNEL_CEILING_ENV, None)
        super().tearDown()

    def test_default_is_permissive(self):
        self.assertIsNone(memsom_ingest.channel_ceiling())
        # endorsed allowed when no ceiling set
        ids = memsom_ingest.ingest_text(self.conn, "endorsed line one two", "endorsed")
        self.assertEqual(len(ids), 1)

    def test_ceiling_blocks_over_rank_channel(self):
        os.environ[memsom_ingest.CHANNEL_CEILING_ENV] = "user"
        self.assertEqual(memsom_ingest.channel_ceiling(), memsom.RANK["user"])
        # user and below are fine
        self.assertEqual(memsom_ingest.enforce_channel_ceiling("user"), "user")
        self.assertEqual(memsom_ingest.enforce_channel_ceiling("external"), "external")
        # endorsed exceeds the ceiling -> refused
        with self.assertRaises(ValueError):
            memsom_ingest.enforce_channel_ceiling("endorsed")
        with self.assertRaises(ValueError):
            memsom_ingest.ingest_text(self.conn, "attacker endorsed text", "endorsed")

    def test_ceiling_accepts_numeric_value(self):
        os.environ[memsom_ingest.CHANNEL_CEILING_ENV] = "1"
        self.assertEqual(memsom_ingest.channel_ceiling(), 1)
        with self.assertRaises(ValueError):
            memsom_ingest.enforce_channel_ceiling("user")  # rank 2 > 1

    def test_ceiling_invalid_value_raises(self):
        os.environ[memsom_ingest.CHANNEL_CEILING_ENV] = "bogus"
        with self.assertRaises(ValueError):
            memsom_ingest.channel_ceiling()

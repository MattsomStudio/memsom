...
Shipped default: memsom does NOT touch keep_alive — the request body OMITS
the key entirely so Ollama applies its own native default (model stays warm).
When MEMDAG_OLLAMA_KEEP_ALIVE is set, every memsom call to the Ollama API
(/api/generate via memsom_llm and memsom_compact, /api/embeddings via
memsom_retrieve) must stamp that value into the request body — 0 = unload
immediately after the call, "10m" = hold warm longer.

No real Ollama needed: urllib.request.urlopen is patched in all network
tests (same FakeResponse pattern as test_memsom_llm.py).  No DB needed.

Run:
  python -W error::DeprecationWarning -m unittest discover \
      -s <repo> -p test_memsom_keepalive.py \
      -t <repo> -v
"""
...
class TestWithKeepAlive(EnvIsolation):
    ...
    def test_unset_leaves_body_untouched(self):
        body = memsom_llm._with_keep_alive({"model": "m"})
...
# ---------------------------------------------------------------------------
# 2. /api/generate via memsom_llm.llm_compose
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. /api/embeddings via memsom_retrieve._call_ollama_embed
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. /api/generate via memsom_compact._llm_summarize
# ---------------------------------------------------------------------------
...
class TestCompactSummaryCarriesKeepAlive(EnvIsolation):
    ...
    def _body_for_summary(self):
        ...
        with patch("urllib.request.urlopen",
            ...
            memsom_compact._llm_summarize(self.ROWS)
        return json.loads(captured["req"].data.decode("utf-8"))

...
class TestDigestRedaction(Base):
    ...
    def test_stem_renders_then_drops_on_redact(self):
        from memsom.distill import digest as memsom_digest
...
class TestEndToEndDemo(unittest.TestCase):
    ...
    def cli(self, *args, expect=0):
        r = subprocess.run([sys.executable, "-m", "memsom", *args],
                           capture_output=True, text=True, encoding="utf-8",
                           env=self.env, cwd=str(HERE.parent), timeout=60)
    ...
    def test_demo_sequence(self):
        ...
        reseed = subprocess.run([sys.executable, "-m", "memsom", "seed", "--offline"],
                                capture_output=True, text=True, encoding="utf-8",
                                env=self.env, cwd=str(HERE.parent), timeout=60)

#!/usr/bin/env python3
"""Tests for memsom_cli — unified CLI surface.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_cli.py \
    -t <repo> -v
"""
...
class Base(unittest.TestCase):
    ...
    def run_cli(self, *argv):
        """Run memsom_cli.main and capture stdout. Raises SystemExit on non-zero exit."""
        buf = io.StringIO()
    ...
    def run_cli_stderr(self, *argv):
        """Run memsom_cli.main; capture stdout + stderr. Returns (stdout, stderr, code)."""
        out_buf = io.StringIO()
        err_buf = io.StringIO()
        code = 0
        try:
...
class TestEnhancedAskQuarantine(Base):
    ...
    def test_quarantined_source_excluded(self):
        ...
        memsom_quarantine.quarantine_node(self.conn, nid2, "suspicious advice")
...
class TestLlmFallback(Base):
    ...
    def test_llm_fallback_to_deterministic(self):
        ...
        if derived:
            memsom.revoke_cascade(self.conn, derived[0], "reset for llm test")
...
class TestEmptyPoolRefusal(Base):
    ...
    def test_empty_pool_exits_1_stores_nothing(self):
        ...
        memsom_quarantine.quarantine_node(self.conn, nid, "test quarantine")
...
class TestCoreDelegation(Base):
    ...
    def test_explain_contains_frozen_strings(self):
        ...
        text, used = memsom.compose("nebula", memsom.live_sources(self.conn))
    ...
    def test_add_stamps_channel_label(self):
        ...
        self.assertEqual(memsom.get_node(self.conn, nid)["label"], memsom.RANK["external"])
...
class TestAskWithoutRetrieveUnchanged(Base):
    ...
    def test_ask_without_retrieve_unchanged(self):
        ...
        if derived:
            memsom.revoke_cascade(self.conn, derived[0], "reset for test")

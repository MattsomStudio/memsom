#!/usr/bin/env python3
"""Tests for memsom_compact — the CONSOLIDATION ENGINE.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_compact.py \
    -t <repo> -v
"""
...
# ---------------------------------------------------------------------------
# Base (verbatim copy from test_memsom.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mock urlopen helper (mirrors test_memsom_ingest.TestIngestUrl._make_mock_urlopen)
# ---------------------------------------------------------------------------
...
class TestMigrateAddsArchivedColumns(Base):
    def test_migrate_adds_archived_columns(self):
        ...
        self.assertTrue(memsom_schema.column_exists(self.conn, "nodes", "archived"))
        self.assertTrue(memsom_schema.column_exists(self.conn, "nodes", "archived_at"))
...
class TestCompactMintsSemanticWithEdgesToEveryEpisode(Base):
    def test_compact_mints_semantic_with_edges_to_every_episode(self):
        ...
        minted = memsom_compact.compact(self.conn)

        self.assertEqual(len(minted), 1)
        semantic_id = minted[0]

        parent_ids = {p[0] for p in memsom.parents_of(self.conn, semantic_id)}
...
class TestLabelIsMinOfParentsNoLaundering(Base):
    def test_label_is_min_of_parents_no_laundering(self):
        ...
        node = memsom.get_node(self.conn, minted[0])
...
class TestExtractiveSummaryDeterministic(Base):
    def test_extractive_summary_deterministic(self):
        ...
        result1 = memsom_compact.extractive_summary(rows)
        result2 = memsom_compact.extractive_summary(rows)
    ...
    def test_compact_deterministic_across_fresh_dbs(self):
        ...
        minted_a = memsom_compact.compact(self.conn)
        self.assertEqual(len(minted_a), 1)
        content_a = memsom.get_node(self.conn, minted_a[0])["content"]
        ...
        try:
            ...
            try:
                with conn2:
                    memsom.insert_node(conn2, EP1, "user", memsom.RANK["user"])
                    memsom.insert_node(conn2, EP2, "user", memsom.RANK["user"])
                    memsom.insert_node(conn2, EP3, "user", memsom.RANK["user"])
                minted_b = memsom_compact.compact(conn2)
                self.assertEqual(len(minted_b), 1)
                content_b = memsom.get_node(conn2, minted_b[0])["content"]
            finally:
                conn2.close()
        finally:
            tmp2.cleanup()
...
class TestCompactSecondRunNoop(Base):
    def test_compact_second_run_noop(self):
        ...
        minted_first = memsom_compact.compact(self.conn)
        self.assertEqual(len(minted_first), 1)

        # Second run: archived episodes no longer candidates
        minted_second = memsom_compact.compact(self.conn)
...
class TestLlmFallsBackToExtractive(Base):
    def test_llm_falls_back_to_extractive_when_ollama_unreachable(self):
        ...
        expected_summary = memsom_compact.extractive_summary(expected_rows)

        with patch("urllib.request.urlopen",
                   side_effect=urllib.error.URLError("refused")):
            minted = memsom_compact.compact(self.conn, llm=True)

        self.assertEqual(len(minted), 1)
        content = memsom.get_node(self.conn, minted[0])["content"]
...
class TestArchivedExcludedFromDefaultRetrieval(Base):
    def test_archived_excluded_from_default_retrieval(self):
        ...
        results_before = memsom_retrieve.retrieve(
        ...
        memsom_compact.compact(self.conn)

        results_after = memsom_retrieve.retrieve(
            self.conn, "nebula lighthouse certificate"
        )
...
class TestExplainAndBlameStillWalkArchived(Base):
    def test_explain_and_blame_still_walk_archived(self):
        ...
        parent_ids = {p[0] for p in memsom.parents_of(self.conn, semantic)}
        self.assertEqual(parent_ids, {n1, n2, n3})

        # explain output contains each episode id
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            memsom_cli.main(["explain", str(semantic)])
        ...
        with contextlib.redirect_stdout(buf2):
            memsom_cli.main(["blame", str(semantic)])
...
class TestGroupByClaim(Base):
    def test_group_by_claim(self):
        ...
        memsom_corroborate.register_root(self.conn, "rootB", by="test")
        ...
        claim_id = memsom_corroborate.assert_claim(
            self.conn, n1, ("port", "is", "4242"), "rootA"
        )
        memsom_corroborate.assert_claim(
            self.conn, n2, ("port", "is", "4242"), "rootB"
        )

        minted = memsom_compact.compact(self.conn, group_by="claim")
        self.assertEqual(len(minted), 1)

        parent_ids = {p[0] for p in memsom.parents_of(self.conn, minted[0])}
...
class TestInvalidGroupByRaises(Base):
    def test_invalid_group_by_raises(self):
        with self.assertRaises(ValueError):
            memsom_compact.compact(self.conn, group_by="bogus")
...
class TestListArchived(Base):
    def test_list_archived(self):
        ...
        archived = memsom_compact.list_archived(self.conn)
...
class TestRedactEpisodeScrubsSemantic(Base):
    ...
    def test_redact_episode_scrubs_semantic(self):
        ...
        memsom_federation.migrate(self.conn)
        ...
        newly = memsom_redact.redact_node(self.conn, id_a, "audit-poc2")
        ...
        changeset = memsom_federation.export_changeset(self.conn, origin="test-machine")
...
class TestCompactConfHighWater(Base):
    def test_compact_propagates_secret_conf_not_public(self):
        ...
        memsom_confid.classify(self.conn, s1, 'secret')   # conf=2
        memsom_confid.classify(self.conn, s2, 'secret')   # conf=2
        minted = memsom_compact.compact(self.conn, group_by='similarity', min_group=2, sim_threshold=0.0)
        ...
        parents = [p[0] for p in memsom.parents_of(self.conn, nid)]
        self.assertEqual(sorted(parents), sorted([s1, s2]))         # edge to every episode
        self.assertEqual(node['label'], min(memsom.RANK['user'], memsom.RANK['user']))  # min(parents)
        for sid in (s1, s2):
...
class TestCompactPurgesRetrievalIndex(Base):
    ...
    def test_archive_and_purge_are_one_transaction(self):
        ...
        real_table_exists = memsom_schema.table_exists
        ...
        with patch.object(memsom_schema, "table_exists", side_effect=boom):

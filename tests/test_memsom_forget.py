"""Tests for memsom_forget — the RS/SS forgetting layer ported into memsom.
...
class Adapter(unittest.TestCase):
    ...
    def add_mem(self, stem, channel, salience=None, body="content body", **cols):
        ...
        content = "---\n" + "\n".join(fm) + "\n---\n\n" + body + "\n"
        nid = memsom.insert_node(self.conn, content, channel, source_ref=f"memory:{stem}")
        if cols:
    ...
    def test_inventory_scoped_to_memory_nodes_only(self):
        ...
        memsom.insert_node(self.conn, "vault note", "user", source_ref="vault:Security/note.md")
        self.conn.commit()

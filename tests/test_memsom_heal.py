#!/usr/bin/env python3
"""Tests for memsom_heal — self-healing: detect-and-report + deterministic rebuild.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_heal.py \
    -t <repo> -v
"""
...
class TestConfMismatchMultiHop(Base):
    def test_multihop_conf_mismatch_flagged(self):
        ...
        d1, _ = memsom.derive_node(self.conn, "hop one", [a])
        d2, _ = memsom.derive_node(self.conn, "hop two", [d1])
        memsom_confid.recompute_conf_all(self.conn)  # both -> conf 3
        ...
        flagged = {v["node"] for v in memsom_heal.check(self.conn)
...
class TestIntegrityViolationFoundAndFixed(Base):
    ...
    def test_integrity_violation_found_and_fixed(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived", [u])  # label 2

        # Corrupt: raw UPDATE to wrong label
        with self.conn:
            self.conn.execute("UPDATE nodes SET label=3 WHERE id=?", (d,))
        self.assertEqual(memsom.get_node(self.conn, d)["label"], 3)

        # check must find the mismatch
        violations = memsom_heal.check(self.conn)
        ...
        # rebuild fixes it
        summary = memsom_heal.rebuild_derived(self.conn)
        self.assertGreaterEqual(summary["integrity_fixed"], 1)

        # check now clean for (b)
        after = memsom_heal.check(self.conn)
        ...
        self.assertEqual(memsom.get_node(self.conn, u)["label"], 2)
        self.assertEqual(memsom.get_node(self.conn, u)["channel"], "user")
...
class TestRedactedWithContentFoundAndFixed(Base):
    ...
    def test_redacted_with_content_found_and_fixed(self):
        # Add the redacted column manually (memsom_redact module is absent in this install)
        memsom_schema.add_column(self.conn, "nodes", "redacted",
        ...
        self.assertEqual(memsom.get_node(self.conn, nid)["content"], "")
...
class TestLiveChildOfTombstonedFoundAndFixed(Base):
    ...
    def test_live_child_of_tombstoned_found_and_fixed(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived d", [a])
        ...
        self.assertEqual(memsom.get_node(self.conn, d)["tombstoned"], 0)
        ...
        # d is now tombstoned
        d_node = memsom.get_node(self.conn, d)
        ...
        a_node = memsom.get_node(self.conn, a)
...
# ---------------------------------------------------------------------------
# Test 5: conf mismatch found and fixed (requires memsom_confid)
# ---------------------------------------------------------------------------
...
class TestConfMismatchFoundAndFixed(Base):
    ...
    Skipped when memsom_confid is not installed.
    """

    def setUp(self):
        super().setUp()
        # Skip if memsom_confid is not available
        if memsom_heal.memsom_confid is None:
            self.skipTest("memsom_confid not installed")

    def test_conf_mismatch_found_and_fixed(self):
        mc = memsom_heal.memsom_confid
        ...
        d, _ = memsom.derive_node(self.conn, "child derived", [a])
...
class TestSourcesNeverModified(Base):
    ...
    def test_sources_never_modified(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived", [u, e])
        ...
        memsom_heal.rebuild_derived(self.conn)
...
class TestCleanDbChecksClean(Base):
    ...
    def test_clean_db_checks_clean(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived answer", [u, e])
        ...
        summary2 = memsom_heal.rebuild_derived(self.conn)
...
# ---------------------------------------------------------------------------
# Test 8: defensive import — memsom_confid = None
# ---------------------------------------------------------------------------

class TestDefensiveImport(Base):
    """test_defensive_import:
    monkeypatch memsom_heal.memsom_confid = None; check and rebuild still run
    (conf checks skipped, no crash).
    """

    def test_defensive_import(self):
        original_confid = memsom_heal.memsom_confid
        try:
            # Simulate memsom_confid being absent
            memsom_heal.memsom_confid = None

            u = self.add("user source", "user")
            d, _ = memsom.derive_node(self.conn, "derived", [u])
            ...
            memsom_heal.memsom_confid = original_confid

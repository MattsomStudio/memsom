#!/usr/bin/env python3
"""Tests for memsom_llm — the opt-in LLM answer path.

Run:
  python -W error::DeprecationWarning -m unittest discover \
      -s <repo> -p test_memsom_llm.py \
      -t <repo> -v
...
# ---------------------------------------------------------------------------
# Fixture — two live source rows (same style as TestCompose.SRC in test_memsom.py)
# ---------------------------------------------------------------------------
...
class TestLlmCompose(unittest.TestCase):
    ...
    def test_cite_overlap_floor_env_clamped(self):
        ...
        try:
            ...
            self.assertEqual(memsom_llm._cite_overlap_floor(), 1.0)
            os.environ["MEMDAG_LLM_CITE_OVERLAP"] = "-1"    # < 0 -> clamp to 0.0
            self.assertEqual(memsom_llm._cite_overlap_floor(), 0.0)
            os.environ["MEMDAG_LLM_CITE_OVERLAP"] = "garbage"  # unparsable -> default
            self.assertEqual(memsom_llm._cite_overlap_floor(),
                             memsom_llm.DEFAULT_CITE_OVERLAP)
...
class TestResolve(unittest.TestCase):
    ...
    def test_defaults_used_when_nothing_set(self):
        m, url = resolve()
        self.assertEqual(m, memsom_llm.DEFAULT_MODEL)
        self.assertEqual(url, memsom_llm.DEFAULT_URL)
...
class TestMigrate(unittest.TestCase):
    ...
    def test_migrate_is_noop(self):
        ...
        mock_conn = MagicMock()
        result = memsom_llm.migrate(mock_conn)

#!/usr/bin/env python3
"""Tests for memsom_quarantine.

Run:
  python -W error::DeprecationWarning -m unittest discover \
      -s <repo> -p test_memsom_quarantine.py \
      -t <repo> -v
"""
...
class Base(unittest.TestCase):
    ...
    def derive(self, content, parent_ids):
        nid, _label = memsom.derive_node(self.conn, content, parent_ids)
        return nid
...
class TestConsolidateQuarantinesExternalDerived(Base):
    ...
    def test_consolidate_quarantines_external_derived(self):
        ...
        results = memsom_quarantine.consolidate(self.conn)
        ...
        # Demo-shape guard: memsom.live_sources still returns BOTH sources (frozen code
...
class TestQuarantinedExcludedFromPool(Base):
    ...
    def test_quarantined_excluded_from_pool(self):
        ...
        unq = memsom_quarantine.live_unquarantined_sources(self.conn)
...
class TestPromoteRefusesThenSucceeds(Base):
    ...
    def test_promote_refuses_then_succeeds(self):
        ...
        with self.assertRaises(ValueError) as cm:
            memsom_quarantine.promote(self.conn, d, "matt")
        ...
        flipped = memsom_quarantine.quarantine_node(self.conn, d2, "test manual quarantine")
        self.assertTrue(flipped)

        # promote(d2) should SUCCEED: has endorsed ancestor, no external ancestor
        memsom_quarantine.promote(self.conn, d2, "matt")
        ...
        memsom_quarantine.quarantine_node(self.conn, d3, "test no-endorsed quarantine")
        with self.assertRaises(ValueError) as cm2:
            memsom_quarantine.promote(self.conn, d3, "matt")
...
class TestConsolidateIdempotent(Base):
    ...
    def test_consolidate_idempotent(self):
        ...
        first = memsom_quarantine.consolidate(self.conn)
        self.assertTrue(any(r[0] == d for r in first))

        second = memsom_quarantine.consolidate(self.conn)
...
class TestQuarantineNodeValidation(Base):
    ...
    def test_quarantine_node_validation(self):
        ...
        with self.assertRaises(ValueError):
            memsom_quarantine.quarantine_node(self.conn, 9999, "bad id")

        # ValueError on tombstoned node
        u = self.add("User source to be tombstoned.", "user")
        memsom.revoke_cascade(self.conn, u, "gone")
        with self.assertRaises(ValueError):
            memsom_quarantine.quarantine_node(self.conn, u, "already dead")

        # False on already-quarantined
        good = self.add("Live node for double-quarantine test.", "user")
        first = memsom_quarantine.quarantine_node(self.conn, good, "first time")
        self.assertTrue(first)
        second = memsom_quarantine.quarantine_node(self.conn, good, "second time")
...
class TestReleaseAuditBreadcrumb(Base):
    ...
    def test_release_audit_breadcrumb(self):
        ...
        memsom_quarantine.quarantine_node(self.conn, d, "pre-promote test")
...
class TestCliSubcommands(Base):
    ...
    def run_main(self, *argv):
        ...
        with contextlib.redirect_stdout(buf):
            memsom_quarantine.main(list(argv))
        return buf.getvalue()
    ...
    def test_cli_promote_gate_failure(self):
        ...
        memsom_quarantine.consolidate(self.conn)

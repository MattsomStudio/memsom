#!/usr/bin/env python3
"""Tests for memsom_recompute — multi-hop integrity recompute.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_recompute.py \
    -t <repo> -v
"""
...
class TestChainRefloors(Base):
    ...
    def test_chain_refloors(self):
        ...
        b, _ = memsom.derive_node(self.conn, "mid", [a])   # label 0
        c, _ = memsom.derive_node(self.conn, "tip", [b])   # label 0
        ...
        self.assertEqual(memsom.get_node(self.conn, b)["label"], 2)
        self.assertEqual(memsom.get_node(self.conn, c)["label"], 2)
...
class TestDiamondTrueMin(Base):
    ...
    def test_diamond_true_min(self):
        ...
        b, _ = memsom.derive_node(self.conn, "from endorsed", [e])   # label 3
        c, _ = memsom.derive_node(self.conn, "from external", [x])   # label 0
        d, _ = memsom.derive_node(self.conn, "join", [b, c])          # label 0
        ...
        self.assertEqual(memsom.get_node(self.conn, b)["label"], 3)
...
class TestElevatedNodeIsFixedPoint(Base):
    ...
    def test_elevated_fixed_point_and_child_rises(self):
        ...
        b, _ = memsom.derive_node(self.conn, "derived b", [a])  # label 0
        c, _ = memsom.derive_node(self.conn, "derived c", [b])  # label 0
        ...
        # Verify stored
        self.assertEqual(memsom.get_node(self.conn, b)["label"], 3)
        self.assertEqual(memsom.get_node(self.conn, c)["label"], 3)
...
class TestSourceLabelsNeverTouched(Base):
    ...
    def test_source_labels_never_touched(self):
        ...
        old, new = memsom_recompute.recompute_node(self.conn, e)
        self.assertEqual((old, new), (3, 3))

        old, new = memsom_recompute.recompute_node(self.conn, u)
        self.assertEqual((old, new), (2, 2))

        old, new = memsom_recompute.recompute_node(self.conn, x)
        ...
        for nid, channel in [(e, "endorsed"), (u, "user"), (x, "external")]:
            ...
            self.assertEqual(node["label"], memsom.RANK[channel],
...
class TestIdempotent(Base):
    ...
    def test_idempotent(self):
        ...
        b, _ = memsom.derive_node(self.conn, "mid", [a])
        c, _ = memsom.derive_node(self.conn, "tip", [b])

        # Re-stamp a to 2
        with self.conn:
            self.conn.execute("UPDATE nodes SET label=2 WHERE id=?", (a,))

        first = memsom_recompute.recompute_all(self.conn)
        self.assertTrue(first, "first call should have changes")

        second = memsom_recompute.recompute_all(self.conn)
...
class TestTombstonedParentsExcluded(Base):
    ...
    def test_tombstoned_excluded_and_zero_live_fallback(self):
        ...
        d, _ = memsom.derive_node(self.conn, "derived d", [a, x])  # label 0
        ...
        old, new = memsom_recompute.recompute_node(self.conn, d)
        ...
        old2, new2 = memsom_recompute.recompute_node(self.conn, d)
...
class TestUnknownIdRaises(Base):
    ...
    def test_unknown_id_raises(self):
        with self.assertRaises(ValueError):
            memsom_recompute.recompute_label(self.conn, 999)
...
class TestDeepChainNoRecursionError(Base):
    ...
    def test_deep_chain_no_recursion_error(self):
        ...
        # Spot-check tip
        tip = memsom.get_node(self.conn, 1501)

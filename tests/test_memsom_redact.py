#!/usr/bin/env python3
"""Tests for memsom_redact — run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_redact.py \
    -t <repo> -v
"""
...
class TestContentDestroyedShapeIntact(Base):
    ...
    def test_content_destroyed_shape_intact(self):
        ...
        b, _ = memsom.derive_node(self.conn, "derived content from a", [a])

        # Pre-capture b's metadata and edge count
        node_b_pre = memsom.get_node(self.conn, b)
        ...
        memsom_redact.redact_node(self.conn, b, "pii")

        node_b = memsom.get_node(self.conn, b)
        # Payload destroyed
...
class TestDescribeShowsRedactedAndWalks(Base):
    ...
    def test_describe_shows_redacted_and_walks(self):
        ...
        b, _ = memsom.derive_node(self.conn, "derived from endorsed source", [a])

        memsom_redact.redact_node(self.conn, a, "privacy-violation")

        lines = memsom_redact.describe(self.conn, a)
        combined = "\n".join(lines)
        self.assertIn("[REDACTED", combined)
        self.assertIn("privacy-violation", combined)

        # The child node can still walk to its parent (shape intact)
        parents = memsom.parents_of(self.conn, b)
...
class TestComposeNeverLeaks(Base):
    ...
    def test_compose_never_leaks(self):
        ...
        # (b) compose over memsom.live_sources (which still includes the row)
        # must not surface the secret text because content is now ''
        all_sources = memsom.live_sources(self.conn)
        text, _used = memsom.compose("nebula configuration", all_sources)
...
class TestRedactIsNotRevoke(Base):
    ...
    def test_redact_is_not_revoke(self):
        ...
        node_y = memsom.get_node(self.conn, y)
...
class TestCascadeRedactsDescendants(Base):
    ...
    def test_cascade_redacts_descendants(self):
        ...
        b, _ = memsom.derive_node(self.conn, "left branch b", [a])
        c, _ = memsom.derive_node(self.conn, "right branch c", [a])
        d, _ = memsom.derive_node(self.conn, "join node d", [b, c])

        result = memsom_redact.redact_node(self.conn, a, "spill", cascade=True)
...
class TestReredactNoop(Base):
    ...
    def test_reredact_noop(self):
        ...
        first_result = memsom_redact.redact_node(self.conn, a, "original-reason")
        ...
        second_result = memsom_redact.redact_node(self.conn, a, "new-reason")
...
class TestUnknownIdRaises(Base):
    ...
    def test_unknown_id_raises(self):
        with self.assertRaises(ValueError):
            memsom_redact.redact_node(self.conn, 99999, "test")

        with self.assertRaises(ValueError):
            memsom_redact.is_redacted(self.conn, 99999)

        with self.assertRaises(ValueError):
            memsom_redact.describe(self.conn, 99999)
...
class TestCliDryRunThenYes(Base):
    ...
    def test_cli_cascade_dry_run_and_yes(self):
        ...
        b, _ = memsom.derive_node(self.conn, "cascade child content", [a])
...
class TestDefaultRedactCascadesToDescendants(Base):
    ...
    def test_default_redact_cascades_to_descendants(self):
        ...
        text, used = memsom.compose("password secret", sources)
        ...
        derived_id, _ = memsom.derive_node(self.conn, text, used)
        ...
        newly = memsom_redact.redact_node(self.conn, src_id, "audit-poc1")
        ...
        text_after, _ = memsom.compose("password secret", sources_after)
        ...
        parents = memsom.parents_of(self.conn, derived_id)

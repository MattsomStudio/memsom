#!/usr/bin/env python3
"""Tests for memsom_relate — GraphRAG-safe associative edges.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_relate.py \
    -t <repo> -v
"""
...
class Base(unittest.TestCase):
    ...
    def relate(self, a, b, kind="relates-to"):
        memsom_relate.relate(self.conn, a, b, kind)
...
class TestNeighborhoodReturnRelated(Base):
    def test_neighborhood_returns_related(self):
        ...
        results = memsom_relate.neighborhood(self.conn, u, hops=1)
...
class TestPoisonedNeighborCannotRide(Base):
    def test_poisoned_neighbor_cannot_ride(self):
        ...
        results = memsom_relate.neighborhood(self.conn, good, hops=3, min_integrity=1)
        ids = {d["id"] for d in results}
        ...
        results0 = memsom_relate.neighborhood(self.conn, good, hops=3, min_integrity=0)
...
class TestPathMinValue(Base):
    def test_path_min_value(self):
        ...
        results = memsom_relate.neighborhood(self.conn, e, hops=2, min_integrity=0)
...
class TestBestPathWins(Base):
    def test_best_path_wins(self):
        ...
        results = memsom_relate.neighborhood(self.conn, start, hops=2, min_integrity=0)
        by_id = {d["id"]: d for d in results}
...
class TestClearanceFiltersConf(Base):
    def test_clearance_filters_conf(self):
        ...
        memsom_confid.classify(self.conn, secret_node, "secret")

        # clearance=internal(1): secret node hidden
        results_low = memsom_relate.neighborhood(
        ...
        results_high = memsom_relate.neighborhood(
...
class TestUndirected(Base):
    def test_undirected(self):
        ...
        results = memsom_relate.neighborhood(self.conn, b, hops=1)
        ids = {d["id"] for d in results}
...
class TestTombstonedAndQuarantinedExcludedAndNonconducting(Base):
    def test_tombstoned_excluded_and_nonconducting(self):
        ...
        with self.conn:
            self.conn.execute(
                ...
                (memsom.now_iso(), "test tombstone", middle)
            )

        results = memsom_relate.neighborhood(self.conn, start, hops=3, min_integrity=0)
        ids = {d["id"] for d in results}
    ...
    def test_quarantined_excluded_and_nonconducting(self):
        ...
        memsom_quarantine.quarantine_node(self.conn, middle, "suspected")
...
class TestRelEdgesDistinctFromProvenance(Base):
    def test_rel_edges_distinct_from_provenance(self):
        ...
        memsom.revoke_cascade(self.conn, x, "test revoke")
        ...
        parents = memsom.parents_of(self.conn, y)
...
class TestRelateValidationAndIdempotence(Base):
    def test_relate_unknown_id_raises(self):
        ...
        with self.assertRaises(ValueError):
            memsom_relate.relate(self.conn, a, 99999)
        with self.assertRaises(ValueError):
            memsom_relate.relate(self.conn, 99999, a)

    def test_relate_self_raises(self):
        """ValueError if a == b."""
        a = self.add("Node a content long enough here.", "user")
        with self.assertRaises(ValueError):
            memsom_relate.relate(self.conn, a, a)
    ...
    def test_relate_idempotent(self):
        ...
        memsom_relate.relate(self.conn, a, b)
        memsom_relate.relate(self.conn, a, b)  # idempotent: INSERT OR IGNORE

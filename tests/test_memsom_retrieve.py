#!/usr/bin/env python3
"""Tests for memsom_retrieve.

Run:
  python -W error::DeprecationWarning -m unittest discover \
    -s <repo> -p test_memsom_retrieve.py \
    -t <repo> -v
"""
...
# ---------------------------------------------------------------------------
# Base test setup (mirrors test_memsom.py Base)
# ---------------------------------------------------------------------------
...
class TestTokenize(unittest.TestCase):
    def test_lowercase_alnum(self):
        tokens = memsom_retrieve.tokenize("Hello, World! 123")
        self.assertIn("hello", tokens)
        self.assertIn("world", tokens)
        self.assertIn("123", tokens)

    def test_stopwords_removed(self):
        tokens = memsom_retrieve.tokenize("how should I configure a network")
    ...
    def test_stem_length_6(self):
        tokens = memsom_retrieve.tokenize("configure")
        self.assertIn("config", tokens)

    def test_empty_text(self):
        self.assertEqual(memsom_retrieve.tokenize(""), [])

    def test_repeats_preserved(self):
        tokens = memsom_retrieve.tokenize("nebula nebula")
...
class TestMigrate(Base):
    def test_tables_created(self):
        for table in ("postings", "docstats", "embeddings"):
            self.assertTrue(
                memsom_schema_table_exists(self.conn, table),
...
def memsom_schema_table_exists(conn, name):
    row = conn.execute(
...
class TestIndexNode(Base):
    ...
    def test_index_node_skips_agent_derived(self):
        ...
        nid, _ = memsom.derive_node(self.conn, "answer about nebula", [src])
        memsom_retrieve.index_node(self.conn, nid)
        row = self.conn.execute(
    ...
    def test_index_node_skips_tombstoned(self):
        ...
        memsom.revoke_cascade(self.conn, nid, "gone")
        memsom_retrieve.index_node(self.conn, nid)
        row = self.conn.execute(
            "SELECT 1 FROM docstats WHERE node_id=?", (nid,)
        ).fetchone()
    ...
    def test_index_node_stores_embedding_when_ollama_up(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _make_fake_embed(fake_vec)):
            memsom_retrieve.index_node(self.conn, nid)
        row = self.conn.execute(
            "SELECT vec FROM embeddings WHERE node_id=?", (nid,)
        ).fetchone()
        self.assertIsNotNone(row)
        recovered = memsom_retrieve._blob_to_vec(row[0])
...
class TestIndexAll(Base):
    def test_reindex_builds_postings_for_all_sources(self):
        ...
        derived_id, _ = memsom.derive_node(self.conn, "derived answer", [ids[0]])
...
class TestBM25(Base):
    ...
    def test_bm25_ranks_relevant_above_noise(self):
        ...
        results = memsom_retrieve.bm25(self.conn, "nebula overlay network lighthouse", k=5)
    ...
    def test_bm25_returns_empty_when_no_postings(self):
        results = memsom_retrieve.bm25(self.conn, "anything", k=5)
    ...
    def test_bm25_returns_empty_for_stopword_only_query(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            memsom_retrieve.index_all(self.conn)
        results = memsom_retrieve.bm25(self.conn, "how should the", k=5)
        self.assertEqual(results, [])
    ...
    def test_bm25_respects_k(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            memsom_retrieve.index_all(self.conn)
        results = memsom_retrieve.bm25(self.conn, "nebula network configuration", k=3)
    ...
    def test_bm25_scores_positive(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            memsom_retrieve.index_all(self.conn)
        results = memsom_retrieve.bm25(self.conn, "nebula network", k=5)
...
class TestVectorSearch(Base):
    def test_vector_search_returns_empty_when_ollama_down(self):
        def _failing(*a, **kw):
            raise OSError("connection refused")
        with patch.object(memsom_retrieve, "_call_ollama_embed", _failing):
            results = memsom_retrieve.vector_search(self.conn, "nebula config", k=5)
        self.assertEqual(results, [])
    ...
    def test_vector_search_returns_empty_when_no_embeddings(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _make_fake_embed(fake_vec)):
            results = memsom_retrieve.vector_search(self.conn, "nebula", k=5)
    ...
    def test_vector_search_cosine_order(self):
        ...
        with self.conn:
            self.conn.execute(
                ...
                (nid_a, memsom_retrieve._embed_model(), 4, _pack_vec(vec_a)),
            )
            self.conn.execute(
                "INSERT OR REPLACE INTO embeddings(node_id, model, dim, vec) VALUES (?,?,?,?)",
                (nid_b, memsom_retrieve._embed_model(), 4, _pack_vec(vec_b)),
            )

        # Query vector close to vec_b (y direction)
        query_vec = [0.0, 0.9, 0.1, 0.0]
        with patch.object(memsom_retrieve, "_call_ollama_embed", _make_fake_embed(query_vec)):
            results = memsom_retrieve.vector_search(self.conn, "dummy query", k=5)
    ...
    def test_vector_search_respects_k(self):
        for i in range(5):
            ...
            with self.conn:
                self.conn.execute(
                    "INSERT OR REPLACE INTO embeddings(node_id, model, dim, vec) VALUES (?,?,?,?)",
                    (nid, memsom_retrieve._embed_model(), 4, _pack_vec(vec)),
                )
        query_vec = [1.0, 0.0, 0.0, 0.0]
        with patch.object(memsom_retrieve, "_call_ollama_embed", _make_fake_embed(query_vec)):
            results = memsom_retrieve.vector_search(self.conn, "test", k=2)
...
class TestRRF(unittest.TestCase):
    def test_rrf_deterministic(self):
        ...
        r1 = memsom_retrieve._rrf_fuse(bm25_list, vec_list)
        r2 = memsom_retrieve._rrf_fuse(bm25_list, vec_list)
    ...
    def test_rrf_boosts_overlap(self):
        ...
        fused = memsom_retrieve._rrf_fuse(bm25_list, vec_list)
    ...
    def test_rrf_empty_inputs(self):
        self.assertEqual(memsom_retrieve._rrf_fuse([], []), [])
        r = memsom_retrieve._rrf_fuse([(1, 2.0)], [])
...
class TestRetrieve(Base):
    ...
    def test_retrieve_respects_clearance(self):
        ...
        memsom_confid.classify(self.conn, sec_id, "secret")

        def _no_embed(*a, **kw):
            raise OSError("down")
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            memsom_retrieve.index_all(self.conn)
            results = memsom_retrieve.retrieve(
    ...
    def test_retrieve_excludes_quarantined(self):
        ...
        memsom_quarantine.quarantine_node(self.conn, nid_quar, "test quarantine")
    ...
    def test_retrieve_excludes_redacted(self):
        ...
        memsom_redact.redact_node(self.conn, nid_red, "test redact")
    ...
    def test_retrieve_excludes_tombstoned(self):
        ...
        memsom.revoke_cascade(self.conn, nid_dead, "killed")
    ...
    def test_retrieve_empty_db(self):
        def _no_embed(*a, **kw):
            raise OSError("down")
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            results = memsom_retrieve.retrieve(self.conn, "anything", k=5)
        self.assertEqual(results, [])
    ...
    def test_redaction_deindexes_and_never_surfaces(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            ...
            memsom_redact.redact_node(self.conn, nid, "test redact")
            ...
            leaky = memsom_retrieve.retrieve(
    ...
    def test_deindex_node_no_op_without_schema(self):
        ...
        try:
            ...
            try:
                nid = memsom.insert_node(conn, "x", "user", memsom.RANK["user"])
                memsom_retrieve.deindex_node(conn, nid)  # must not raise / create tables
                self.assertFalse(
                    memsom_schema.table_exists(conn, "postings"))
            finally:
                conn.close()
        finally:
            os.environ.pop("MEMDAG_DB", None)
    ...
    def test_retrieve_with_vector_cosine_ordering(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _fake_embed):
            memsom_retrieve.index_all(self.conn)
            results = memsom_retrieve.retrieve(self.conn, "nebula overlay lighthouse", k=5)
...
class TestCLI(Base):
    ...
    def test_retrieve_cli_no_results(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            with contextlib.redirect_stdout(buf):
                memsom_retrieve.main(["retrieve", "nebula"])
        out = buf.getvalue()
    ...
    def test_retrieve_cli_with_results(self):
        ...
        with patch.object(memsom_retrieve, "_call_ollama_embed", _no_embed):
            with contextlib.redirect_stdout(buf):
                memsom_retrieve.main(["retrieve", "nebula overlay lighthouse", "--k", "5"])
        out = buf.getvalue()
...
class TestCosineHelper(unittest.TestCase):
    def test_identical_vectors(self):
        v = [1.0, 2.0, 3.0]
        self.assertAlmostEqual(memsom_retrieve._cosine(v, v), 1.0, places=5)

    def test_orthogonal_vectors(self):
        a = [1.0, 0.0]
        b = [0.0, 1.0]
        self.assertAlmostEqual(memsom_retrieve._cosine(a, b), 0.0, places=5)

    def test_zero_vector(self):
        a = [0.0, 0.0]
        b = [1.0, 0.0]
        self.assertEqual(memsom_retrieve._cosine(a, b), 0.0)

    def test_opposite_vectors(self):
        a = [1.0, 0.0]
        b = [-1.0, 0.0]
        self.assertAlmostEqual(memsom_retrieve._cosine(a, b), -1.0, places=5)

    def test_mismatched_lengths(self):
        self.assertEqual(memsom_retrieve._cosine([1.0], [1.0, 2.0]), 0.0)
...
class TestVecPackUnpack(unittest.TestCase):
    def test_roundtrip(self):
        ...
        blob = memsom_retrieve._vec_to_blob(vec)
        recovered = memsom_retrieve._blob_to_vec(blob)
        self.assertEqual(len(recovered), len(vec))
        for a, b in zip(recovered, vec):
            self.assertAlmostEqual(a, b, places=5)

    def test_empty_vector(self):
        blob = memsom_retrieve._vec_to_blob([])
        recovered = memsom_retrieve._blob_to_vec(blob)
